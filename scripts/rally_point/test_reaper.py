...
class ReaperDelegatesAtFullTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.fake.write_text(
            ...
            "'schema':'agent-rally.command.version.v1','data':{'version':{'version':'test'}}})); "
            "raise SystemExit(0)\n"
            "if a == ['whoami','--json']:\n"
            "    print(json.dumps({'ok':True,'product':'rally',"
            "'schema':'agent-rally.command.whoami.v1','data':{'whoami':"
            "{'repo_root':repo,'repo_id':'repo_fake','room_id':'room-test',"
            "'worktree':repo,'build_id':'build-test',"
            "'host_runtime':{'ambiguous':False}}}})); raise SystemExit(0)\n"
            "if a[:3] == ['status','--json','read']:\n"
            "    print(json.dumps({'ok':True,'product':'rally',"
            "'schema':'agent-rally.command.status_read.v1',"
            "'data':{'status_read':{'states':[]}}})); raise SystemExit(0)\n"
            "if a[:1] == ['sessions']:\n"
