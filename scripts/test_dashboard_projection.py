...
def test_stale_top_level_done_does_not_override_live_execution(tmp_path: Path) -> None:
    state = _closed_run_state(
        outcome="partial",
        phases={"assess": "complete", "plan": "complete"},
        learn={},
    )
    state["phase"] = "done"
    state["execution"] = {"build_loop_id": _CLOSED_RUN_ID, "phase": "execute"}
