...
- **Load short-term working context (Pillar 0 — runs BEFORE memory bootstrap)**: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/load_current.py --workdir "$PWD" --json` — cheap, local-FS-only read of `.build-loop/context/current.md` so the agent resumes from the prior session's working state immediately. The returned envelope (`{exists, parsed: {phase, run_id, next_action, links_down[], pointers[]}, warm_read_latency_ms, reasons[]}`) feeds the heavy bootstrap below with entry pointers DOWN into long-term memory. Absent / corrupt → `exists: false` + `reasons[]`, never blocks. Mirror `warm_read_latency_ms` into `state.json.assess.workingContextLatencyMs`.
