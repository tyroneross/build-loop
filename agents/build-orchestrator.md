...
- **Load memory** — executable read protocol (full detail in `references/memory-systems.md` §"Read protocol — Phase 1 Assess"): (0) read canonical long-term memory from `~/dev/git-folder/build-loop-memory/indexes/` plus `projects/<slug>/...` via `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/memory_facade.py recall --query "<goal-keywords>" --limit 10`; (1) `Read(".build-loop/state.json")` inspect `runs[-3:]`; (2) `Skill("build-loop:debugging-memory")` with `intent: "list-recent"`; (3) `backend_health.py` health-check, write to `state.json.architecture.backendHealth`.
