...
- **Intent confidence + optional intent-explorer** (lightweight, default-skips): immediately after intent.md is written, run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/intent_confidence.py --goal "<goal>" --workdir "$PWD" --json` and mirror to `state.json.intentConfidence`. When `should_explore: true`, dispatch `Skill("build-loop:intent-explorer")` once, passing the signals + current `intent.md`. The skill appends an `## Exploration findings` section to `intent.md`, mirrors a compact summary to `state.json.exploration`, and returns a one-paragraph summary that routes to Phase 4 Review-G's `## Notes from judges` block. The skill NEVER calls `AskUserQuestion`, never produces a `## Held` entry — advisory per `feedback_advisory_checks_are_automated`. Default verdict is `high` (skip), so the auto-execute fast path is unaffected. Phase 2 Plan's "fork on uncertainty" rule consumes `exploration.approach_options` when `exploration.confidence_after` remains medium/low. Helper failure → treat as `should_explore: false` and log one warning; never blocks.
