...
- **G. Report** (final pass only) — scorecard, run entry via `write_run_entry.py`, debugger outcomes, episodic memory capture, deployment policy gate, post-deploy verification gate (below). Report sections in order: `## Done` (verified + Auto-Resolve auto + `[warn]` items), `## Held` (confirm verdicts), `## Blocked` (block verdicts), `## Status markers` (✅/⚠️/❓). Every verified/known claim MUST include `observer=... method=... artifact=...`; "verified by xcodebuild" alone is not enough. Multi-chunk or parallel runs MUST include `merge_plan:` with `clean_against`, `conflicts_with`, and `suggested_order`. Before emitting the report, write the draft to a temp file and run `python3 scripts/build_report_lint.py <report.md> --json`; exit 1 routes back to report revision, exit 2 is a lint outage recorded under `## Done` as `[warn] build-report-lint skipped (<reason>)`. Forbidden: "Open Recommendations" headers, "Want me to X?" / "Should I Y?" phrasing, lists inviting operator selection. Empty categories: `_(none)_`.
