...
- **Synthesis-presence routing** (REVISED 2026-05-07 round-4 — Phase 1 routing rule, the default-tier flip): when a plan exists at this point in Phase 1, count its `synthesis_dimensions:` entries by calling the helper from the existing parser — `python3 -c 'from pathlib import Path; import sys; sys.path.insert(0, "${CLAUDE_PLUGIN_ROOT}/scripts"); from plan_verify import count_synthesis_dimensions; print(count_synthesis_dimensions(Path("<plan-path>")))'` (or import directly when running in-process). Do NOT invent a second parser; reuse `count_synthesis_dimensions()` so vague-value linting and presence routing share one block-walker. **Threshold (post-experiment flip): count >= 1 (any synthesis_dimensions entry) escalates the commit to `tier: thinking`.** Sonnet fan-out remains the default ONLY for plans with empty or absent `synthesis_dimensions` blocks — i.e., truly mechanical work (formatter runs, dependency bumps, mechanical renames, doc-only edits). Write the verdict to `state.json.synthesisDensity` as `{count: N, escalated: true|false, reason: "synthesis-class work — defaulting to thinking-tier" | "no synthesis decisions — fan-out OK"}`. **Routing target is `tier: thinking`, never a hardcoded model name** — Phase 3 resolves the identifier through the same tier abstraction used by the C5 halt-and-ask resolver (`state.json.config.modelOverrides.thinking` → orchestrator frontmatter `model:` → fail-loud if neither resolves). Why this default flip: the n=6 A/B experiment (2026-05-07, `~/dev/research/topics/synthesis-decision-delegation/experiment-2026-05-07/`) demonstrated Sonnet implementers surface ~40% of architectural decisions Opus implementers surface even with full scaffolding (synthesis_dimensions blocks, attestation envelopes, halt-and-ask backstop). Defaulting to `tier: thinking` for any plan with synthesis_dimensions trades ~28% token cost for ~60% better recall on synthesis-class decisions — quality-first orientation. The C3/C4/C5 backstops remain in place as defense-in-depth, but they were demonstrated to be insufficient alone: β didn't reliably recognize architectural decisions as novel-class, so the halt-and-ask never fired on them. Effect on Phase 3: when `synthesisDensity.escalated == true`, the orchestrator does NOT dispatch parallel implementer subagents for that plan; it executes the chunks inline at `tier: thinking` (single-context). The dual-mode dispatch table still applies — escalation overrides the default fan-out path on a per-plan basis. Skip this step cleanly when no plan file exists yet (re-evaluate at the end of Phase 2 if needed).
