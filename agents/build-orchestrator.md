...
- **E. Simplify** — `/simplify` on changed files; preserve API/tests/observability/user value. **Telemetry (mandatory, every Review pass, all builds — measurement, not a factor):** after E completes this pass, record what E inspected via `update_execution_state(state_path, 'review_e_pass', files_scanned=[<files E actually inspected this pass>], is_final=<True iff this is the FINAL Review pass>)`. This records what E *did*; it MUST NOT change what E does. On Iterate re-entry under the F2 cadence, E naturally scans fewer/non-first-pass files — that delta is the recorded signal.
