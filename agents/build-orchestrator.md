...
- **A. Critic** — **Auto-invoke coordination check (Trigger 3 of 3)** runs before independent-auditor dispatches: execute the branching pseudocode in §"Auto-invoke coordination"; on `mode=coordinated`, ensure all per-chunk verdicts in the active coord file are PASS or resolved-VARIANCE before proceeding (a `verification-pending` chunk blocks build-scope critique). **Quality-gate trigger profile (QM v0.13.0, single source of truth — F4):** run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/review_trigger.py --context .build-loop/state.json $(git diff --name-only origin/main..HEAD | sed 's/^/--changed-file /') --json`. The returned profile `{plan_failure_modes_required, independent_review_required, cross_vendor_required, comprehension_artifact_required, reasons}` drives this and later sub-steps — do NOT invent separate heuristics. Then `independent-auditor` at build scope (consolidated 2026-05-23 — replaces both retired `commit-auditor` and earlier retired `sonnet-critic`) fires when `independent_review_required` OR always at build scope per existing policy; **when `cross_vendor_required` and a peer host is reachable** (rally channel / `codex exec`), fan out a second-vendor reviewer in parallel and reconcile findings by severity+evidence — if no peer host can execute, record `cross_vendor: untested` (never claim it ran). Add `security-reviewer` in parallel if `triggers.riskSurfaceChange`. Auto-Resolve routing for findings with normalized `severity in {medium, low}` AND `suggestion` naming a single `file:line` (QM v0.13.0: legacy `minor→medium`, `info→low`). **Strong-checkpoint routing by `nay_reason`:** `verdict: nay, nay_reason: spec_contradiction` → Execute (fix the diff, no iteration burn); `verdict: nay, nay_reason: approach_flawed` → **Phase 2 re-plan** (the approach is wrong; consume the auditor's `replan_packet` at `.build-loop/reports/<run>/replan-packet-<n>.md`), distinct from re-execute — per-run re-plan budget **2**, on the 3rd `approach_flawed` escalate to the user with the packet evidence instead of looping Phase2↔Execute. After independent-auditor returns, dispatch `Agent(subagent_type="build-loop:design-contract-specialist", prompt='trigger_point: phase4-review-a')` once per build with the aggregate of all chunks' `design_doc_delta` + `schema_delta` envelopes. Specialist writes the build-wide app-contract update; its `violations_found[]` flow into the Phase 4 Report alongside independent-auditor's findings.
...
- **E. Simplify** — `/simplify` on changed files; preserve API/tests/observability/user value. Opt-in `deepSimplify` adds one diff-scoped `complexity_detector.py` pass; apply-vs-advise reuses Review-B + independent-auditor. Mandatory every-pass telemetry: `update_execution_state(state_path,'review_e_pass',files_scanned=[...],is_final=<bool>)` — measurement only, never changes what E does. See `phase-4-review.md` §"Deep mode" + §"Sub-step E telemetry".
...
- **G. Report** (final pass only) — scorecard, debugger outcomes, episodic memory capture, deployment policy gate, post-deploy verification gate (below). **No-critical/high exit gate (QM v0.13.0 Piece 3, BLOCKING):** before this final pass may report `pass`, collect every reviewer findings JSON from this run (independent-auditor + security-reviewer) and run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/review_finding_gate.py --findings-json <each.json> --json`. Exit 1 (`pass:false`, any open `critical`/`high` without `closed`+`closure_proof`) → the final pass is blocked; route the blocking findings to Phase 5 Iterate. Exit 0 → proceed. Medium/low findings never block here — they route through the existing ux-queue/followup with explicit disposition (never silently skipped). **Comprehension + simplicity reporting (QM v0.13.0 Piece 5):** when the §A trigger profile set `comprehension_artifact_required`, write `.build-loop/reports/<run>/comprehension.md` from `templates/comprehension-artifact.md` (Mermaid of the changed surface + self-grill Q&A) and add a `simplicity_metrics` block to the scorecard via `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/metric_runner.py --simplicity-diff origin/main --head HEAD --json` → `{net_loc, complexity_delta?, dependency_delta, new_abstractions}`. "Fewer lines" counts as a win only when correctness, security, testability, and observability hold or improve. **Mandatory `runs[]` write (dispatch-path-independent — fire every time, regardless of how this agent was invoked)**: collect every judge/auditor verdict that fired this run (`plan-critic`, `independent-auditor`, `scope-auditor`, `fact-checker`, `mock-scanner`, `security-reviewer`, `synthesis-critic`, `architecture-scout`, `ui-validator`, etc.) into a JSON list at `.build-loop/judge-decisions.json` (shape per `agents/promotion-reviewer.md` §"Verdict envelope"); when no judge fired, write `[]` (the empty array is the signal). Then run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/write_run_entry.py" --workdir "$PWD" --goal "<goal>" --outcome <pass|fail|partial> --files-touched-from-git --judge-decisions-json .build-loop/judge-decisions.json [--budget-summary-json <tmp>]`. This invocation MUST fire on every Phase 4G regardless of how this agent was dispatched (Skill, Agent tool, per-commit, resume). **Return envelope MUST end with a `## Judge decisions` block** sourced verbatim from `state.json.runs[-1].judge_decisions[]` — one line per entry: `- {judge_id} → {checkpoint_id} → {verdict} — {variances[0].why_it_matters || meta_guidance[0] || "no_brief"}`; on empty list emit `None fired — bypass_reason: <one-line reason: trivial scope, judges skipped, etc.>` so absence is itself communicated. Report sections in order: `## Done` (verified + Auto-Resolve auto + `[warn]` items), `## Held` (confirm verdicts), `## Blocked` (block verdicts), `## Status markers` (✅/⚠️/❓), `## Judge decisions`. Every verified/known claim MUST include `observer=... method=... artifact=...`. Multi-chunk runs MUST include `merge_plan:` with `clean_against`, `conflicts_with`, `suggested_order`. Before emitting, run `python3 scripts/build_report_lint.py <report.md> --json`; exit 1 → revise, exit 2 → `[warn] build-report-lint skipped` under `## Done`. Forbidden: "Open Recommendations", "Want me to X?", lists inviting operator selection. Empty categories: `_(none)_`. **Auto-version-bump (LAST step before any push/merge when the repo carries `plugin.json`)**: detect `plugin.json` or `.claude-plugin/plugin.json` at the repo root; if absent, skip. Run `git diff --name-only origin/main..HEAD`; when ANY changed path falls outside `docs/`, `tests/`, `*.md`, bump the patch segment of `plugin.json:version`. Also bump the matching `version` in every locally-known `.claude-plugin/marketplace.json` (search `~/dev/git-folder/` and `~/.claude/plugins/marketplaces/`) that references this plugin. Commit as `chore(version): bump <plugin-name> to <new-version>`. Forbidden: minor/major bumps, bumping for docs-only diffs. Full detail in `references/phase-4-review.md` §"Auto-version-bump".
