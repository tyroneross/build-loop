...
- **Architecture baseline + blast-radius** (native skills, fire unconditionally): invoke `Skill("build-loop:architecture-scan")` to refresh data, then `Skill("build-loop:architecture-impact")` on up to 5 highest-risk components. Before invoking architecture-scan, check `state.json.architecture.stale`; if true and ACP older than 5 min, await scan completion (default) or pass `--no-arch-await` to override. If `triggers.promptAuthoring` or `triggers.promptEditingExisting` is true, also invoke `mcp__plugin_navgator__llm_map`. Each native skill has its own no-op pre-flight when `.navgator/architecture/index.json` is missing — emits a one-line note and exits without blocking. Compact summary lands in `.build-loop/state.json.architecture.{scan,impact}`. Phase 2 Plan consults this for scoping. **Memory baseline**: invoke `Skill("build-loop:debugging-memory")` with `intent: "list-recent"` to surface recent project incidents (one-line summary). The memory-first gate runs unconditionally on every Review-B failure later — this Assess call is for context priming.
...
- **D. Fact-Check**: dispatch `fact-checker` + `mock-scanner` in parallel. **Plus** when code changed in this build, dispatch `Skill("build-loop:architecture-rules")` in parallel (no plugin gate — the native skill no-ops cleanly when `.navgator/architecture/index.json` is absent). For cross-layer changes, escalate to `Skill("build-loop:architecture-review")` for the full 5-phase integrity review. **Plus the new gates from SKILL.md §Sub-step D**:
...
- **Stuck-iteration escalation (always on)** — at the START of every Iterate attempt, run the cascade in order. Stop at the first rule that fires:
  ...
  3. **Architecture impact pre-step (cross-layer failures)**: if the failing criterion's `files_touched` cross 2+ layers (per `.navgator/architecture/file_map.json` lookup), invoke `Skill("build-loop:architecture-impact")` against each affected component BEFORE any escalation. Forces the orchestrator to confirm whether the failing fix is actually cross-layer or just adjacent files in the same component. If the layer-crossing is real, the plan for the next Iterate attempt MUST include each downstream component flagged by the impact analysis as a coordinated change.
