...
- **B. Validate**: code graders → LLM-as-judge. If `availablePlugins.ibr` and UI work, invoke `ibr:design-validation` for web or `ibr:native-testing` for mobile. Collect evidence. On any FAIL, run memory-first gate.
