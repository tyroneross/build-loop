...
- **Architecture baseline + blast-radius** (architecture-scout subagent, fires unconditionally): dispatch `Agent(subagent_type="build-loop:architecture-scout", prompt='task: baseline')`. The scout decides native vs NavGator per task, runs the scan + impact + ACP build, and returns a ≤500-word envelope. Before dispatch, check `state.json.architecture.stale`; if true and ACP older than 5 min, the scout will await scan completion (default) — pass `task: baseline; no_arch_await: true` to override. If `triggers.promptAuthoring` or `triggers.promptEditingExisting` is true, also invoke `mcp__plugin_navgator__llm_map` (LLM-map is not in the scout's task set yet). Cache the envelope to `.build-loop/architecture/scout-cache/baseline.json` for Phase 2 plan consumption. The native engine no-ops cleanly when no source files match — scout emits a `findings[].kind: "warning"` row but does not block. Compact summary lands in `.build-loop/state.json.architecture.{scan,impact}`. **Memory baseline**: invoke `Skill("build-loop:debugging-memory")` with `intent: "list-recent"` to surface recent project incidents (one-line summary). The memory-first gate runs unconditionally on every Review-B failure later — this Assess call is for context priming.
...
- Each agent gets: task description, relevant file paths, integration contract, relevant fallback snippets, an intent packet from `.build-loop/intent.md` explaining how that task fits the north star, a MECE ownership packet (`owns`, `does not own`, `interface contract`, `integration checkpoint`), and an `architecture_context:` block read verbatim from `.build-loop/architecture/scout-cache/chunk-<N>.json` (Phase 2's fan-out cache). Implementers treat that block as authoritative blast-radius information — they MUST flag any change that exits the slice in their return envelope. Do NOT dispatch the scout again in Phase 3.
...
- **B. Validate**: code graders → LLM-as-judge. **IBR-first when present and UI work**: load `Skill("build-loop:ibr-bridge")` and run the quick-pass BEFORE any other validator:
  ...
  - **Plugin-tests advisory check (auto-runs when build touches plugin metadata; non-blocking)** — if Phase 3 Execute's diff contains any of these path globs, run `Skill("build-loop:plugin-tests")` as part of Validate:
    ...
    - `hooks/hooks.json`
    Concretely run from the repo root: `for t in scripts/test_skill_resolution.py scripts/test_plugin_manifest.py scripts/test_mcp_registration.py scripts/test_trigger_phrases.py scripts/test_bridge_preflight.py; do python3 "$t" || EXIT=1; done; exit ${EXIT:-0}`. **Findings are advisory, not blocking.** Record results to `.build-loop/state.json.pluginTests` as `{exit, failingScripts: [...], details}`. Surface a one-line summary in the Review-F Report (`✅ plugin-tests: 31/31 pass` or `⚠️ plugin-tests: 2 advisories — see state.json.pluginTests`). Do NOT route to Iterate on exit 1; do NOT gate the build. The static tier produces heuristic findings (regex/pattern matches over markdown frontmatter) — false positives are common (3 hit on initial rollout: legitimate user-invocable bridge, filesystem-preflight bridge, PRD-absence prose). User decides whether each finding is a real defect or a false positive. Hard-blocking would just teach the loop's users to disable the gate. Exit 2 (verifier outage) is logged identically to exit 1 — distinguished only in `details`.
  - Collect evidence. On any FAIL, run the memory-first gate.
  - **Memory-first gate (always on)** — runs on every Review-B criterion failure with an error-like signal (exception, test failure, build error). Skip when failure is expected and mapped (TDD "tests must fail until impl complete") or iteration is from user feedback rather than a reproducible bug. Steps:
    ...
    3. **Invoke `Skill("build-loop:debugging-memory")`** with `{ symptom, budget: 2500 }` — native skill (under `skills/debugging/memory/`). It calls the `search` MCP and returns a verdict (`KNOWN_FIX` / `LIKELY_MATCH` / `WEAK_SIGNAL` / `NO_MATCH`).
...
- **D. Fact-Check**: dispatch `fact-checker` + `mock-scanner` in parallel. **Plus** when code changed in this build, dispatch `Agent(subagent_type="build-loop:architecture-scout", prompt='task: review-rules')` in parallel — the scout runs the rules check, diffs against `.episodic/architecture/known_violations.json`, writes decisions for new violations via `scripts/capture_arch_violation.py` (Chunk 6), and returns a `route:` recommendation in `follow_up`. If `route: "iterate"`, route the scout's findings into Iterate's prioritized work list. For cross-layer changes, escalate to `Skill("build-loop:architecture-review")` for the full 5-phase integrity review. **Plus the new gates from SKILL.md §Sub-step D**:
...
- **Stuck-iteration escalation (always on)** — at the START of every Iterate attempt, run the cascade in order. Stop at the first rule that fires:
  ...
  3. **Architecture impact pre-step (cross-layer failures)**: if the failing criterion's `files_touched` cross 2+ layers (per `.build-loop/architecture/file_map.json` lookup, falling back to `.navgator/architecture/file_map.json`), dispatch `Agent(subagent_type="build-loop:architecture-scout", prompt='task: iterate-subgraph, failing_files: [<files>]')` BEFORE any escalation. Scout returns `fix_scope_files` — the union of same-component + direct-downstream files that MUST be touched together. Forces the orchestrator to confirm whether the failing fix is actually cross-layer or just adjacent files in the same component. If the layer-crossing is real, the plan for the next Iterate attempt MUST include each file in `fix_scope_files` as a coordinated change.
  4. **2 consecutive same-root-cause failures** → parallel multi-domain assessment via `Skill("build-loop:debugging-assess")` — native skill (under `skills/debugging/assess/`). Fans out to relevant domain assessors (api / database / frontend / performance) in parallel. **Model override**: explicitly pass `model: sonnet` to each domain assessor via the subagent dispatch to avoid 4 parallel Opus invocations from your Opus 4.7 tier. Only escalate individual assessors to Opus if their initial output flags `confidence: low` or `needs_judgment: true`. Aggregate ranked findings; use the top action as the next Iterate plan.
  5. **3 consecutive same-criterion failures** → causal-tree investigation via `Skill("build-loop:debugging-debug-loop")` — native skill (under `skills/debugging/debug-loop/`). Do not attempt a 4th fix without it. The skill runs its own 7-phase cycle (investigate → hypothesize → fix → verify → score → critique → report) with up to 5 internal iterations. When it returns, validate against build-loop's original Review-B criteria. If still failing after 5 internal debug-loop iterations, hard-stop and escalate to user.
...
- **Partition for fan-out**: group entries by disjoint `files_touched` (no overlapping files). Dispatch mechanism depends on whether you can spawn subagents:

  **Top-level mode** (you were invoked directly via `/build-loop:run` or the user's session, NOT via `Agent(...)` from a parent): dispatch up to 4 `implementer` subagents in parallel via `Agent(subagent_type="build-loop:implementer", ...)` — the agent definition lives at `${CLAUDE_PLUGIN_ROOT}/agents/implementer.md`. Hard cap from `~/.claude/CLAUDE.md` §Sub-Agents. Sequential groups process after the parallel batch.

  **Subagent mode** (you were spawned via `Agent(...)` and the no-sub-sub-agents rule from `~/.claude/CLAUDE.md` §Sub-Agents applies): the `Agent` tool is unavailable to you. **Do NOT halt — degrade gracefully to inline-implementer mode**. Iterate the queue serially, applying each entry's `proposed_fix` yourself using the protocol from `${CLAUDE_PLUGIN_ROOT}/agents/implementer.md` (read every file in `files_touched` first; refuse `architecture_impact: true`; stay in scope; verify locally with typecheck/lint/adjacent-tests/re-grep before declaring fixed; return the same structured status: fixed/partial/scope_breach/etc.). You lose parallelism in this mode but the work still gets done with the same quality bar. Surface the degradation in your Phase 4 Sub-step F Report: `⚠️ Phase 5 ran in subagent mode — no parallel fan-out. For full parallelism, invoke /build-loop:run directly in a top-level session.`
  ...
  - `deferred_architecture` → move entry to Review-F deferred list (Sub-step F surfaces for explicit user confirmation)
  - `evidence_stale` → re-run `ux_triage.py --clear` to regenerate the queue, then re-pass
  - `plan_malformed` → same as `evidence_stale` (regenerate via `ux_triage.py --clear`); log the malformed entry's id to `.build-loop/state.json.malformedPlans[]`
  ...
  - `concurrent_modification_detected` → abort the current parallel batch immediately, surface in Review-F. Indicates the orchestrator's partition logic broke (overlapping `files_touched` slipped through) — a real bug, not transient
...
- **Append a run entry to `.build-loop/state.json.runs[]`** — delegate to the deterministic writer; do NOT hand-write JSON. The writer generates the `run_id`, handles the atomic append, preserves all legacy top-level keys, and fans out per-experiment `applied` rows with correct `co_applied_experimental_artifacts[]` confound tracking. Schema source-of-truth lives in the script (`scripts/write_run_entry.py`).
  ...
  Capture `RUN_ID` from stdout and cite it in the scorecard. `--outcome` must be one of `pass|fail|partial`. `--files-touched-from-git` uses `state.json.preBuildSha` (stamp this at Phase 1 ASSESS); falls back silently if the sha is absent. `--security-findings-json` reads the same file Review-A logged MEDIUM/LOW findings to (CRITICAL/HIGH route back to Execute and are resolved before reaching Review-F); the script silently treats a missing file as no findings, so always pass the flag — it's a safe no-op on builds where `triggers.riskSurfaceChange` was false. Persisting findings into `state.json.runs[].security_findings[]` is what activates the `recurring-pattern-detector` `security_finding` class in Phase 6 Learn. Exit codes: `0` ok, `1` validation error (fix the args and retry — do NOT fall back to hand-writing JSON, which bypasses the file lock), `2` filesystem error (retry once; if it persists, log to `.build-loop/state.json.escalations` and surface to the user).
- **Store resolved debugger incidents and report outcomes** (debugger is bundled with build-loop — no plugin gate). Use the native `Skill("build-loop:debugging-store")`; full procedure also lives in `Skill("build-loop:debugging-memory")` §"Review-F outcome feedback":
  - For each newly resolved Review-B/Iterate failure: invoke `build-loop:debugging-store` (which calls the `store` MCP) with `{symptom, root_cause, fix, tags: ["build-loop", project, layer], files}`
...
2. Dispatch `recurring-pattern-detector` (Haiku) — reads `.build-loop/state.json.runs[]`, returns patterns JSON (only `phase_failure` and `manual_intervention` types; `diagnostic_repeat`/`file_churn` were removed to prevent sprawl). In parallel, dispatch `Agent(subagent_type="build-loop:architecture-scout", prompt='task: learn-sync')` — promotes new lessons (Chunk 8) and syncs NavGator lessons into Postgres (Chunk 7); scout no-ops gracefully when those scripts are not yet present.
