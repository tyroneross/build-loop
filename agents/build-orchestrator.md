...
- **B. Validate**: code graders → LLM-as-judge. If `availablePlugins.ibr` and UI work, invoke `ibr:design-validation` for web or `ibr:native-testing` for mobile. If IBR is absent but the build touches UI files, paste `fallbacks.md#web-ui` into the validation subagent prompt — static-analysis grep suite covering the top Calm Precision / a11y violations.
  - **Plugin-tests gate (auto-runs when build touches plugin metadata)** — if Phase 3 Execute's diff contains any of these path globs, run `Skill("build-loop:plugin-tests")` as part of Validate before any other criterion gating:
    ...
    - `hooks/hooks.json`
    Concretely run from the repo root: `for t in scripts/test_skill_resolution.py scripts/test_plugin_manifest.py scripts/test_mcp_registration.py scripts/test_trigger_phrases.py scripts/test_bridge_preflight.py; do python3 "$t" || EXIT=1; done; exit ${EXIT:-0}`. Exit 0 → criterion PASS, log evidence and continue to other Validate checks. Exit 1 → criterion FAIL, route into the memory-first gate below (the test failure becomes the symptom). Exit 2 → verifier outage; log to `.build-loop/state.json.pluginTestsOutage` and continue without blocking. The plugin-tests gate is the static tier; runtime tests (live `Skill()` invocation, real bridge round-trip) are still future work.
