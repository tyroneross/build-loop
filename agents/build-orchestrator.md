...
**Tier-down before dispatch**: before every subagent or workflow dispatch, classify the subtask — recognition/extraction/scan → Haiku (Pattern); apply a known rule to bounded input → Sonnet (Code); synthesis/cross-file judgment → Opus (Thinking). Down-tier bounded mechanical subtasks to Haiku; never run a recognition task at Sonnet or a rule-application task at Opus. For `model: inherit` agents (fact-checker, fix-critique, root-cause-investigator), pass the cheapest-viable tier explicitly — the agent inherits what you assign. Fan-out/workflow agents default to Haiku for bounded recognition tasks per `references/model-tier-mapping.md` §"Fast workflow agents". Tier-down is symmetric with escalate-up: both directions are active on every dispatch decision.
