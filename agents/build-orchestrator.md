...
- **Synthesis-density routing** (NEW 2026-05-07 — Phase 1 routing rule, complements C3/C4/C5): when a plan exists at this point in Phase 1, count its `synthesis_dimensions:` entries by calling the helper from the existing parser — `python3 -c 'from pathlib import Path; import sys; sys.path.insert(0, "${CLAUDE_PLUGIN_ROOT}/scripts"); from plan_verify import count_synthesis_dimensions; print(count_synthesis_dimensions(Path("<plan-path>")))'` (or import directly when running in-process). Do NOT invent a second parser; reuse `count_synthesis_dimensions()` so vague-value linting and density routing share one block-walker. Threshold: **count > 5 (6 or more entries) escalates the commit to `tier: thinking`**. Write the verdict to `state.json.synthesisDensity` as `{count: N, escalated: true|false, reason: "synthesis-dense — count > 5"}`. **Routing target is `tier: thinking`, never a hardcoded model name** — Phase 3 resolves the identifier through the same tier abstraction used by the C5 halt-and-ask resolver (`state.json.config.modelOverrides.thinking` → orchestrator frontmatter `model:` → fail-loud if neither resolves). Why this rule: the deterministic vague-value lint (C3-adjacent), the subjective synthesis-critic (C4), and the halt-and-ask backstop (C5) each catch one decision at a time; a 6+ dimension plan is synthesis-dense at the COMMIT level — fan-out to Sonnet implementers loses cross-dimension coherence even when each individual dimension is well-specified, and that's exactly what Phase 3's inline `tier: thinking` route preserves. Effect on Phase 3: when `synthesisDensity.escalated == true`, the orchestrator does NOT dispatch parallel implementer subagents for that plan; it executes the chunks inline at `tier: thinking` (single-context). The dual-mode dispatch table still applies — escalation overrides the default fan-out path on a per-plan basis. Skip this step cleanly when no plan file exists yet (re-evaluate at the end of Phase 2 if needed).
