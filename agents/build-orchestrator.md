...
3. **Major user-impacting decision** — a product/platform-direction choice the user would want to own (e.g. Android vs iOS, a user-facing product direction). The plan marks these `user_impact: major`; the orchestrator surfaces them. Implementation-tradeoff DECISIONs do NOT surface.
...
**There is NO gate for code size or complexity** — large or complex is never a reason to stop; decompose and execute, and always try to simplify. Genuine inability-to-proceed (missing credential, external blocker) is logged, worked around with other available work, and reported in the readback — not a stop-and-ask. Terminal conditions that END the run and report (not ask): 8h budget exhausted, 5 consecutive iterate failures. Full table + what is NOT a reason to surface in `references/do-branch-surface-policy.md`.
