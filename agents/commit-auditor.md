...
- `chunk_id` — e.g. `c1`
- `diff_sha` — pre-commit local SHA range
- `diff_stat` — `{files_changed, lines_added, lines_removed}`
- `files_owned` — chunk's planned file scope
- `plan_path` — absolute path to the plan / rubric markdown
- `rubric_criteria_ids` — IDs active for this chunk (e.g. `["F2","F5","r4"]`)
...
1. `Read(plan_path)` — load the rubric and any inline criteria.
2. `Read("~/.build-loop/memory/constitution.md")` — full body for cited rules.
3. Form expectations of what the diff should contain.
4. Now `git diff` (via Bash) to see the actual diff for this chunk.
...
```json
{
  ...
  "checkpoint_id": "<run_id>:<chunk_id>:pre-commit",
  ...
  "variances": [
    {
      "id": "v1",
      "spec_ref": "rubric:r4 | constitution:C-AUTH/auth_change_requires_test",
  ...
  "policy_refs": ["rubric:r4", "constitution:C-AUTH/auth_change_requires_test", "memory:feedback_<slug>"]
}
```
...
## Trivial bypass (orchestrator-side; for context)

The orchestrator skips dispatching you when ALL of:
...
When bypassed, the orchestrator logs `{judge_id: "commit-auditor", verdict: "approve", bypass_reason: "trivial", ...}` directly to `judge_decisions[]`. You don't need to do anything in that case — by definition you weren't called.
