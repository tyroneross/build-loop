...
exports.autoExtractPatternIfReady = autoExtractPatternIfReady;
 ...
 */
async function extractPatterns(options = {}) {
    return (0, logger_1.traced)('patterns:extractPatterns', { min: options.min_incidents }, async () => {
        const { min_incidents = 3, min_similarity = 0.7, auto_store = false, config } = options;
        ...
        const existingPatterns = await (0, storage_2.loadAllPatterns)(config);
        ...
        for (const [category, categoryIncidents] of Object.entries(categorized)) {
            if (categoryIncidents.length < min_incidents)
                continue;
        ...
        for (const candidate of candidates) {
            ...
            const patternId = (0, storage_1.generatePatternId)(candidate.category, 'common_fix');
            ...
            if (auto_store) {
                await (0, storage_1.storePattern)(pattern, config);
 ...
 */
function calculateCommonality(incidents) {
 ...
 */
function createPatternFromIncidents(candidate) {
    ...
    const pattern = {
        pattern_id: (0, storage_1.generatePatternId)(category, 'common_fix'),
 ...
 */
function extractDetectionSignature(incidents) {
 ...
 */
function synthesizeSolution(incidents) {
    ...
    const bestApproach = approaches.reduce((longest, current) => current.length > longest.length ? current : longest, '');
 ...
 */
function extractBestCodeExample(incidents) {
    // Find incident with highest confidence and code snippet
    const withCode = incidents.filter(i => i.root_cause.code_snippet);
    if (withCode.length === 0)
        return undefined;
    const best = withCode.reduce((highest, current) => current.root_cause.confidence > highest.root_cause.confidence ? current : highest);
    return best.root_cause.code_snippet;
}
/**
 * Count incident usage by agent
 */
function countByAgent(incidents) {
    const counts = {};
    for (const incident of incidents) {
 ...
 * Extract caveats from incidents
 */
function extractCaveats(incidents) {
    const caveats = [];
 ...
 */
function generatePatternName(category, incidents) {
    const categoryNames = {
 ...
 */
function generatePatternDescription(category, incidents) {
 ...
 */
async function autoExtractPatternIfReady(newIncident, options = {}) {
    return (0, logger_1.traced)('patterns:autoExtract', { category: newIncident.root_cause.category }, async () => {
        const { minSimilar = 3, minQuality = 0.75, config } = options;
        ...
        const similarIncidents = allIncidents.filter(inc => inc.root_cause.category === category &&
        ...
        const existingPatterns = await (0, storage_2.loadAllPatterns)(config);
        const patternId = (0, storage_1.generatePatternId)(category, 'common_fix');
        const exists = existingPatterns.some(p => p.pattern_id === patternId);
        if (exists) {
        ...
        const { storeIncident: updateIncident } = await Promise.resolve().then(() => __importStar(require('./storage')));
 ...
 */
async function suggestPatterns(config) {
