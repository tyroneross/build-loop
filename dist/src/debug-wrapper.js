...
exports.debugWithMemory = debugWithMemory;
exports.storeDebugIncident = storeDebugIncident;
 ...
 */
async function debugWithMemory(symptom, options = {}) {
    const { agent = 'unknown', auto_store = true, min_confidence = 0.7 } = options;
    console.log('\n🧠 Debug With Memory');
    console.log('═══════════════════════════════════════════════\n');
    // Step 1: Check Memory
    console.log('Step 1: Checking memory for similar incidents...');
    const memory = await (0, retrieval_1.checkMemory)(symptom, {
 ...
 *   });
 */
async function storeDebugIncident(session_id, incident_data) {
    ...
    const incident = {
        incident_id: (0, storage_1.generateIncidentId)(category),
        timestamp: Date.now(),
    ...
    // Store incident
    try {
        const result = await (0, storage_1.storeIncident)(incident, { validate_schema: true });
        ...
        const pattern = await (0, pattern_extractor_1.autoExtractPatternIfReady)(incident);
        if (pattern) {
 ...
 */
async function verifyIncidentStorage(incident_id, config) {
 ...
 */
async function saveSessionContext(context, config) {
    const paths = (0, config_1.getMemoryPaths)(config);
    await promises_1.default.mkdir(paths.sessions, { recursive: true });
    const filepath = path_1.default.join(paths.sessions, `${context.session_id}.json`);
    await promises_1.default.writeFile(filepath, JSON.stringify(context, null, 2), 'utf-8');
}
/**
 * Load session context
 */
async function loadSessionContext(session_id, config) {
    const paths = (0, config_1.getMemoryPaths)(config);
    const filepath = path_1.default.join(paths.sessions, `${session_id}.json`);
    try {
        const content = await promises_1.default.readFile(filepath, 'utf-8');
 ...
 */
async function cleanupSession(session_id, config) {
    const paths = (0, config_1.getMemoryPaths)(config);
    const filepath = path_1.default.join(paths.sessions, `${session_id}.json`);
    try {
 ...
 */
function calculateCompleteness(incident_data) {
    const fields = {
        symptom: true,
        root_cause: !!incident_data.root_cause,
        fix: !!incident_data.fix,
...
async function getMemoryStatus() {
    ...
    console.log('═══════════════════════════════════════════════\n');
    const stats = await (0, storage_1.getMemoryStats)();
