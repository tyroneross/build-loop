...
exports.generateDynamicSection = generateDynamicSection;
exports.generateSessionContext = generateSessionContext;
exports.checkFileContext = checkFileContext;
 ...
 */
async function generateDynamicSection(config) {
    const index = await (0, storage_1.loadIndex)(config);
 ...
 */
async function generateSessionContext(config) {
    const index = await (0, storage_1.loadIndex)(config);
    if (!index || index.stats.total_incidents === 0) {
        return {
            ok: true,
 ...
 */
async function checkFileContext(filepath, config) {
    const index = await (0, storage_1.loadIndex)(config);
    if (!index) {
