...
exports.mineAuditTrail = mineAuditTrail;
exports.previewAuditMining = previewAuditMining;
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const storage_1 = require("./storage");
const logger_1 = require("./logger");
const getAuditDir = () => path_1.default.join(process.cwd(), '.claude/audit');
/**
 * Mine audit trail for incidents
 */
async function mineAuditTrail(options = {}) {
    return (0, logger_1.traced)('audit:mineTrail', { days_back: options.days_back }, async () => {
        const { days_back = 30, auto_store = false, min_confidence = 0.7, config } = options;
        ...
        const existing = await (0, storage_1.loadAllIncidents)(config);
        ...
        const novel = filtered.filter(f => f.symptom && f.symptom.length > 10 &&
        ...
        for (const finding of novel) {
            ...
            if (auto_store) {
                await (0, storage_1.storeIncident)(incident, { validate_schema: true, config });
 ...
 */
async function findAuditFiles(days_back) {
    const AUDIT_DIR = getAuditDir();
    const cutoff = Date.now() - (days_back * 24 * 60 * 60 * 1000);
    const files = [];
    try {
        const entries = await promises_1.default.readdir(AUDIT_DIR, { withFileTypes: true });
        for (const entry of entries) {
            if (entry.isDirectory()) {
                // Check subdirectories (like RUN_xxx/)
                const subdir = path_1.default.join(AUDIT_DIR, entry.name);
                const subFiles = await promises_1.default.readdir(subdir);
                for (const subFile of subFiles) {
                    if (subFile.endsWith('.md')) {
                        const fullPath = path_1.default.join(subdir, subFile);
                ...
                const fullPath = path_1.default.join(AUDIT_DIR, entry.name);
                const stats = await promises_1.default.stat(fullPath);
                if (stats.mtimeMs >= cutoff) {
                    files.push(fullPath);
                }
 ...
 */
async function extractIncidentsFromFile(filePath) {
    const findings = [];
    try {
        ...
        console.warn(`⚠️  Could not parse ${path_1.default.basename(filePath)}: ${error}`);
    }
    return findings;
}
/**
 ...
 */
function extractErrorEntries(content) {
    const findings = [];
 ...
 */
function convertFindingToIncident(finding) {
    ...
    const incident = {
        incident_id: (0, storage_1.generateIncidentId)(),
 ...
 */
function inferCategory(symptom, root_cause) {
 ...
 */
function extractTags(symptom, root_cause) {
    const text = `${symptom} ${root_cause || ''}`.toLowerCase();
    const tags = [];
    // Technology tags
    const techTags = [
    ...
    if (text.includes('render'))
        tags.push('rendering');
    if (text.includes('infinite') || text.includes('loop'))
        tags.push('infinite-loop');
    if (text.includes('crash') || text.includes('fail'))
        tags.push('crash');
    if (text.includes('timeout'))
        tags.push('timeout');
    if (text.includes('memory') || text.includes('leak'))
        tags.push('memory-leak');
    if (text.includes('race') || text.includes('concurren'))
        tags.push('concurrency');
    if (text.includes('deadlock'))
        tags.push('deadlock');
    if (text.includes('auth') || text.includes('permission'))
        tags.push('authentication');
    if (text.includes('validation'))
        tags.push('validation');
    if (text.includes('schema') || text.includes('migration'))
        tags.push('schema');
    if (text.includes('search') || text.includes('query'))
        tags.push('search');
    if (text.includes('cron') || text.includes('schedule'))
        tags.push('cron');
 ...
 */
async function previewAuditMining(days_back = 30, config) {
...
//# sourceMappingURL=audit-miner.js.map