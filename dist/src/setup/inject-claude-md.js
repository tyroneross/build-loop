...
exports.injectClaudeMd = injectClaudeMd;
 ...
 */
function generateDynamicSectionSync(projectRoot) {
    ...
    try {
        ...
        if (!index.stats || index.stats.total_incidents === 0)
            return '';
...
async function injectClaudeMd(projectRoot) {
    ...
    if (fs.existsSync(claudeMdPath)) {
        ...
        if (content.includes(DYNAMIC_MARKER_START)) {
            ...
            if (dynamicSection) {
                const regex = new RegExp(`${DYNAMIC_MARKER_START}[\\s\\S]*?${DYNAMIC_MARKER_END}`, 'g');
