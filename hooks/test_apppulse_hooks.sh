#!/usr/bin/env bash
# Fixture-driven tests for the App Pulse hooks:
#   - session-start-apppulse.sh : one-line restore only on delta,
#                                 silent + exit 0 otherwise
#   - pre-edit-apppulse.sh      : cheap revision-stat hint, never blocks
...
SS_HOOK="${SCRIPT_DIR}/session-start-apppulse.sh"
PE_HOOK="${SCRIPT_DIR}/pre-edit-apppulse.sh"
PKG="${REPO_ROOT}/scripts/app_pulse"
