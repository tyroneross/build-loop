...
exports.BrowserTraceAdapter = void 0;
const storage_1 = require("../storage");
const summarizer_1 = require("../summarizer");
class BrowserTraceAdapter {
    constructor() {
        this.source = 'chrome-devtools';
    }
    /**
    ...
    parseChrome(trace) {
        ...
        const significantEvents = trace.traceEvents.filter((e) => e.ph === 'X' && // Complete events
            e.dur && // Has duration
            e.dur > 1000 && // > 1ms
     ...
     */
    summarizeChromeGroup(category, events) {
        const totalDuration = events.reduce((sum, e) => sum + (e.dur || 0), 0);
        const avgDuration = totalDuration / events.length;
        const slowest = events.reduce((max, e) => ((e.dur || 0) > (max.dur || 0) ? e : max), events[0]);
        ...
        return {
            ...
            summary: (0, summarizer_1.createTraceSummary)(category, `${events.length} ${category} events, total ${durationMs.toFixed(0)}ms`, {
                highlights: [
     ...
     */
    mapChromeCategory(cat) {
        if (cat.includes('paint') || cat.includes('layout'))
            return 'ui-render';
        if (cat.includes('v8') || cat.includes('script'))
            return 'custom';
        if (cat.includes('loading'))
     ...
     */
    convertPlaywrightAction(action) {
        ...
        return {
            ...
            summary: (0, summarizer_1.createTraceSummary)(action.name, `Action: ${action.name}`, {
    ...
    convertNetworkError(entry) {
        ...
        return {
            ...
            summary: (0, summarizer_1.createTraceSummary)('Network Error', `${entry.response.status} ${entry.response.statusText}: ${entry.request.method} ${this.truncateUrl(entry.request.url)}`, {
                highlights: [
    ...
    convertConsoleError(log) {
        return {
            ...
            summary: (0, summarizer_1.createTraceSummary)('Console Error', log.text.substring(0, 200), {
                error: {
                    type: 'ConsoleError',
...
exports.BrowserTraceAdapter = BrowserTraceAdapter;
