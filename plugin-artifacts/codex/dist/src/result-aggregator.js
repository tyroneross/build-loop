...
exports.scoreIncident = scoreIncident;
exports.scorePattern = scorePattern;
exports.aggregateResults = aggregateResults;
exports.formatAggregatedResults = formatAggregatedResults;
exports.createQuickSummary = createQuickSummary;
// Scoring weights
 ...
 */
function scoreAssessment(assessment, config = {}) {
    ...
    let score = (SCORING_WEIGHTS.match * matchScore +
 ...
 */
function scoreIncident(incident, config = {}) {
    const finalConfig = { ...DEFAULT_CONFIG, ...config };
 ...
 */
function scorePattern(pattern, matchScore = 0.5, config = {}) {
    const finalConfig = { ...DEFAULT_CONFIG, ...config };
...
function deduplicateItems(items, threshold) {
    ...
    for (const item of items) {
        const isDuplicate = result.some(existing => calculateSimilarity(existing, item) >= threshold);
        if (!isDuplicate) {
            result.push(item);
        }
    }
 ...
 */
function aggregateResults(assessments, incidents, patterns, config = {}) {
    const finalConfig = { ...DEFAULT_CONFIG, ...config };
    // Score all items
    ...
    const domains = [...new Set(topItems.map(i => i.domain).filter(Boolean))];
 ...
 */
function createQuickSummary(result) {
