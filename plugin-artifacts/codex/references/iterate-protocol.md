...
2. **For each item (sequential — alignment-check is per-item):**
   ...
   d. **Per-item cap — enforced.** Run `autonomy_supervisor.py verdict --item <stable-id> --verdict <verdict> --limit 3` after each item outcome. `action: quarantine` moves the item plus its verdict receipts to follow-up and continues the manifest. A changed verdict resets the consecutive count.
