...
2. **Input**: `.build-loop/intent.md` request contract + `.build-loop/goal.md` + active plan + current report/diagnostics + aligned open queues + the implementer's diff (`git diff HEAD~1` or the changed-file set).
3. **Output**: JSON envelope with `verdict` ∈ {yay, nay, suggest_correction, look_again} + normalized `findings[]` (`severity: critical|high|medium|low`) + `completion_routing`. See `agents/independent-auditor.md` for the full schema.
4. **Routing** (QM v0.13.0 normalized severities; legacy `major→high`, `minor→medium`, `info→low`):
   - `completion_routing.action: return_to_orchestrator` → obey `next_phase` before sub-step B: `replan` repairs a weakened plan; `iterate|execute` fixes the named open items. This route takes precedence over the headline verdict.
   - `verdict: yay` with `completion_routing.action: proceed` → proceed to sub-step B (Validate)
   ...
   - `severity: medium|low` findings that are adjacent/out-of-scope → record in `.build-loop/issues/` and proceed. A `same_intent` finding at any severity without an evidenced `fixed|user_deferred|external_blocked|waived` disposition returns to the orchestrator; `critical|high` also never proceed silently.
...
**Finding and original-intent exit gate (QM v0.13.0 Piece 3, BLOCKING).** Before this final pass may report `pass`, collect every reviewer JSON envelope produced this run (independent-auditor + security-reviewer) and run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/review_finding_gate.py --findings-json <each.json> --json`. It normalizes legacy severities and blocks when either (a) any `critical`/`high` finding remains open or (b) any `same_intent` finding at any severity lacks an evidenced terminal disposition. It also consumes top-level `completion_routing`; an auditor-requested `return_to_orchestrator` cannot be lost when `findings[]` is empty or incomplete. **Exit 1 → obey `orchestrator_route`: `replan` repairs request-contract weakening; otherwise route the named open items to Execute/Phase 5 Iterate.** Only `orchestrator_route.action: proceed` may enter Report. Adjacent/out-of-scope medium/low findings remain nonblocking but still require explicit disposition.
