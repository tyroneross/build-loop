...
3. **Resource-aware parallel agents** where the dependency graph allows. Load `references/resource-aware-execution.md`, resolve the concrete model, then run `scripts/parallelism.py --describe --json` with provider/location, output-size, effort, and agent. Persist the envelope at `state.json.execution.fanout` and dispatch no more than `effective_max`: cloud inference is token-led using measured usage then T-shirt fallback; local inference is CPU-led using model-size core reservations. Both retain config, CPU-headroom, hard-ceiling, and MECE limits. Batch with `plan_batches()` when chunk count exceeds the result.
