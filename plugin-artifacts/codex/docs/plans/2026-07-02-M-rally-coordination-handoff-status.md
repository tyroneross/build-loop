...
**ARP `v0.1.4` released** (tag `v0.1.4`, HEAD `6fbbd95`, release not draft, **all 4 platform binaries published** — Linux `x86_64`/`aarch64`, macOS `aarch64` + `x86_64`, each built from the tag commit `6fbbd95` and provenance-attested). The Intel `x86_64-apple-darwin` binary is **cross-compiled on the arm64 `macos-14` runner** (`file` → `Mach-O x86_64`, runs under Rosetta reporting `0.1.4+6fbbd95`); the original `macos-13` native Intel job starved in GitHub's queue (~5h, never scheduled — GitHub is retiring `macos-13`), so the durable fix drops the Intel-runner dependency entirely. This closes the ledger-corruption saga:
...
|---|---|---|---|
| `release-verify-v0.1.4` | claude_code | done | ✅ **All 4 triples published** from tag `6fbbd95`, release not draft, provenance-attested. Intel `x86_64-apple-darwin` now cross-compiles on `macos-14` (root-cause fix for `macos-13` runner starvation; `release.yml` also pins checkout to the input tag so re-publishes match the tag). Reinstall on any OTHER machines — binary-install lag is the recurring systemic cause. |
