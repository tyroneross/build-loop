...
- Capture app/repo north star and update intent in `.build-loop/intent.md`. Preserve every active result-bearing user clause first under `## Request contract` as stable `RC-*` IDs, then record purpose, primary users, core jobs, user value, and non-goals. Mirror `request_contract[]` into `state.json.intent`; later phases may sequence those outcomes but cannot narrow them without an explicit user decision against the affected ID.
...
- Map every task and acceptance criterion to one or more `RC-*` request-contract IDs. A bounded sample, non-goal, follow-up, or documentation-only result cannot replace an `all`, `full`, `comprehensive`, or `exhaustive` outcome unless the user explicitly changes that ID.
...
**Sub-step A — Critic (adversarial read-only)**: perform a read-only adversarial review of the diff — inline in the lead session, or via an authorized explorer when delegation is permitted. This is the independent-auditor pass: render one of four verdicts (`yay` approve / `nay` reject / `suggest_correction` / `look_again`) after reading the original `RC-*` request contract, goal, active plan, current report, diagnostics, open issue/follow-up/backlog queues, PRD, and constitution. Catch scope drift, missed edge cases, rubric violations, and known same-intent gaps before spending tokens on full validation. A same-intent finding without an evidenced terminal disposition returns to the orchestrator at any severity; `replan` takes precedence when the plan weakened the request. **When Assess flagged a risk-surface change** (new auth, network, persistence, secrets, or external-input surface), also run a security review in this sub-step against OWASP LLM/Agentic/Web top-10 — its critical/high findings feed the finding exit gate and are blocking. Strong-checkpoint findings route back to Execute (no iteration burn); adjacent/out-of-scope guidance findings are logged.
...
- Run `scripts/review_finding_gate.py` over every reviewer JSON envelope and obey `orchestrator_route`. Only `action: proceed` enters Report. `return_to_orchestrator` with `next_phase: replan` repairs request-contract weakening; otherwise fix the named same-intent items in Execute/Iterate. Top-level `completion_routing` is binding even when `findings[]` is empty.
