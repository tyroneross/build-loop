...
- ``pre-push`` — the path-agnostic push choke point (see ``hooks/git/pre-push``).
  It runs composed deploy-HOLD, deterministic TEST, and plugin VERSION gates.
  The test gate was added per the 2026-06-29 RCA so a red commit cannot reach
  origin/main; the version gate prevents an unchanged plugin version from being
  pushed as a new release. All stages ship in
