...
def test_staged_update_to_head_tracked_runtime_path_is_grandfathered(repo: Path) -> None:
    ...
    # the staged pre-commit path; observed stranding 2026-08-28.
    target = repo / ".build-loop" / "backlog" / "item.md"
    target.parent.mkdir(parents=True)
...
def test_all_mode_stays_strict_on_grandfathered_paths(repo: Path) -> None:
    target = repo / ".build-loop" / "backlog" / "item.md"
    target.parent.mkdir(parents=True)
    target.write_text("v1\n")
    _git(["add", "-A"], repo)
    ...
    assert result.returncode == 1
    assert ".build-loop/backlog/item.md" in result.stderr


def test_unborn_branch_grandfathers_nothing(repo: Path) -> None:
    target = repo / ".build-loop" / "new.md"
    target.parent.mkdir(parents=True)
