...
def compose(workdir: Path, *, full_git: bool = False) -> dict:
    ...
    document = _TEMPLATE.format(
        ...
        # Don't print "if a run WAS mid-Execute, resume from the last chunk"
        # underneath a §3 that already states no run is in progress. A cold
        # reader (Opus, 2026-07-26) flagged the pair as a self-contradiction:
        # the doc asserts a state and then hedges it, and could not tell which
        # to believe. Emit the branch that actually applies.
        mid_execute_step=(
            "A run IS in progress — resume from the last committed chunk boundary, "
            "not from the start of the phase."
            if (state.get("execution") or {}).get("phase")
            else "No run is mid-Execute (see §3), so there is no chunk boundary to "
                 "resume from. Any new work starts a NEW run."
        ),
        workdir=str(workdir),
    )
