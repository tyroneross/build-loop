...
Run by .git/hooks/pre-commit. Delegates to check_private_slugs.py.

The pinned path is a FAST PATH, not the only path. Pinning alone dies on every
plugin upgrade: a bump from 0.39.4 to 0.40.0 replaced the cache directory, and
three repos then refused every commit with a FileNotFoundError traceback that
said nothing about private slugs. Resolution now falls back to the repo's own
scripts/ and to the newest version in either plugin cache.

FAILS CLOSED. This is a security guard, so an unresolvable checker refuses the
commit (exit 2) rather than skipping. That is the deliberate difference from the
runtime-memory guard, which may fail open because it is advisory.
"""
import os
import runpy
import sys
...
_CHECKER = {checker!r}
_REL = Path("scripts") / "check_private_slugs.py"


def _version_key(p):
    return [int(c) if c.isdigit() else -1 for c in p.name.split(".")]


def _candidates():
    override = os.environ.get("BUILD_LOOP_SLUG_CHECKER")
    if override:
        yield Path(override)
    yield Path(_CHECKER)
    top = os.environ.get("GIT_TOPLEVEL")
    if not top:
    ...
    if top:
        yield Path(top) / _REL
    for cache in (Path.home() / ".codex" / "plugins" / "cache",
                  Path.home() / ".claude" / "plugins" / "cache"):
        for base in cache.glob("**/build-loop"):
            if not base.is_dir():
                continue
            for v in sorted((d for d in base.iterdir() if d.is_dir()),
...
for _candidate in _candidates():
    try:
        if _candidate.is_file():
            sys.argv = [str(_candidate)]
            runpy.run_path(str(_candidate), run_name="__main__")
            sys.exit(0)
    except SystemExit as exc:
        sys.exit(exc.code or 0)
    except OSError:
        continue
print("private-slug-guard: no check_private_slugs.py found on any known path. "
      "Refusing the commit rather than skipping a security guard.",
      file=sys.stderr)
sys.exit(2)
'''
