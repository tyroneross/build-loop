# Codex: validate the App Pulse cross-tool channel (run this on `main`)

**Why this exists.** App Pulse gives every app a shared channel at
...
in-repo test (`scripts/app_pulse/test_cross_tool.py`) loads the channel code
twice from two different install locations (this repo's `scripts/app_pulse/` and
...
ls ~/.codex/plugins/cache/*/build-loop/*/scripts/app_pulse/checkpoint.py
python3 scripts/check_cache_sync.py --host codex --json
```

**Expected:** the `ls` prints at least one `checkpoint.py` path (App Pulse is in
your cache), and the check exits 0 with an empty `"diffs": []`.

If the `ls` finds nothing, App Pulse is not in your Codex plugin cache yet —
...
`6 passed`. If any test is *skipped* with "no installed plugin cache app_pulse",
...
uv run python -c "
import sys, os
sys.path.insert(0, 'scripts/app_pulse')
...
print('SLUG:', slug)
"
echo "Hand this SLUG back to Claude: codex-apppulse-v4-$STAMP"
```

**Expected:** it prints `WROTE channel: /Users/.../.build-loop/apps/codex-apppulse-v4-<STAMP>`
...
> where that cache is the resolved `scripts/app_pulse`. Running it from
...
1. Step 1 result (App Pulse in your Codex cache? `check_cache_sync` exit code).
...
```bash
rm -rf ~/.build-loop/apps/codex-apppulse-v4-*
```
