...
Codex exposes two primary user-facing Build Loop choices plus one temporary alias:

- `build-loop` — main workflow and router for build, fix, debug, optimize,
  research, knowledge capture, handoff, and plugin-test work
- `repo-maintenance` — repository structure, source-of-truth, artifact retention,
  worktree, branch, stash, and local-main maintenance
- `repo-closeout` — temporary compatibility alias for `repo-maintenance`; do not
  use it in new prompts or automation
...
- `handoff`
- `repo-maintenance`
- `repo-closeout` (temporary compatibility alias)
...
The artifact contains the approved `build-loop` and `repo-maintenance` skill
trees plus the temporary `repo-closeout` compatibility alias, with
`.codex-plugin/plugin.json` rewritten to `"skills": "./skills"`. It must not
contain any other `SKILL.md` files.
