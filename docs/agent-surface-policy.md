...
Codex and ChatGPT install from the repository root — it IS the Codex plugin
surface, with no generated mirror in between. The root carries the full
runtime: hooks, scripts, agents, commands, canonical internal skills,
references, source modules, templates, and package metadata. Its manifest
`.codex-plugin/plugin.json` keeps `"skills": "./codex-skills"`, and that
directory contains only `build-loop/SKILL.md`. Internal `skills/*/SKILL.md`
files remain available to the orchestrator at their canonical paths but are
outside the manifest-declared option root.

The local Codex marketplace (`.agents/plugins/marketplace.json`) declares
`"source": "."`. Release and marketplace updates must keep it there.
...
`npm run codex:sync-cache` syncs the repository root into the Codex cache. The
prune command keeps the current host manifest version and deletes older
verified cache directories for the same plugin. Use `--host codex` or
