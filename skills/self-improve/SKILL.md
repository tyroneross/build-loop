...
**Promotion compares arms, not levels.** The sweep computes a CONTROL mean — the outcome metric over runs in the same window that did not apply this artifact — and promotes only when `observed - control >= target - baseline`. Fewer than 8 control runs means no comparison is possible, reported as `control_missing`. Without this the gate compared the treatment mean to a hand-authored target with nothing measuring what those runs would have scored anyway: this repository's ambient outcome mean is about 0.72 across its non-experimental runs, so any target at or below 0.72 was met by the base rate alone, and an experimental skill fires on the run class its own description matches, so its sample is self-selected.
