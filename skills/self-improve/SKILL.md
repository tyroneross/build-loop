...
**Promotion compares arms, not levels.** The sweep computes a CONTROL mean — the outcome metric over runs in the same window that did not apply this artifact — and promotes only when `observed - control >= target - baseline`. Control runs are bounded to the last 90 days, because runs from a different era of the repository are not a comparison. Fewer than 8 in-window control runs means no comparison is possible, reported as `control_missing`. An experiment measured by a CALLER-supplied metric keeps the level comparison instead: the control arm speaks run outcomes, which share no scale with "seconds to complete", and differencing the two both blocks real improvements and passes flat ones. The receipt's `target_metric.comparison` names which rule ran. Without this the gate compared the treatment mean to a hand-authored target with nothing measuring what those runs would have scored anyway: this repository's ambient outcome mean is about 0.72 across its non-experimental runs, so any target at or below 0.72 was met by the base rate alone, and an experimental skill fires on the run class its own description matches, so its sample is self-selected.
