...
**Promotion compares arms, not levels.** The sweep computes a CONTROL mean — the outcome metric over runs in the same window that did not apply this artifact — and promotes only when `observed - control >= target - baseline`. BOTH arms are bounded to the last 90 days, because runs from a different era of the repository are not a comparison in either direction; out-of-window treatment rows are reported as `treatment_out_of_window`. Fewer than 8 in-window control runs means no comparison is possible, reported as `control_missing`. An experiment measured by a CALLER-supplied metric keeps the level comparison instead: the control arm speaks run outcomes, which share no scale with "seconds to complete", and differencing the two both blocks real improvements and passes flat ones. The receipt's `target_metric.comparison` names which rule ran. A sample that MIXES outcome-derived and caller-supplied rows is reported as `metric_mismatch` rather than graded: averaging the two produces a mean on no scale at all. Without this the gate compared the treatment mean to a hand-authored target with nothing measuring what those runs would have scored anyway: this repository's ambient outcome mean is about 0.72 across its non-experimental runs, so any target at or below 0.72 was met by the base rate alone, and an experimental skill fires on the run class its own description matches, so its sample is self-selected.

An `applied` row whose `metric_value` is not numeric does NOT count toward the sample. Phase 6 Learn reports those as `metric_missing` (with a per-artifact `metric_missing_detail`) on the `sample_sweep` stage of the receipt, so an unmeasurable sample never looks like a young one. **History: this is why the evidence-based promotion path was inert.** `append_experiment_rows` hardcoded `metric_value: None` while `scripts/learn/runner.py` kept only numeric rows, so `len(applied)` was always 0, always below the floor, and the sample-sweep promotion never fired from a real run. The tests missed it because they injected `0.9` into row fixtures no writer produced.
