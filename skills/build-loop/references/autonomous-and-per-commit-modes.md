...
2. For each row in the manifest's `debts[]`, dispatch it using that row's own `dispatch_command` against that row's own `diff_range` (e.g. `Agent(subagent_type="build-loop:independent-auditor")`), append the verdict to `.build-loop/judge-decisions.json` stamped with that row's `run_id` and `diff_range`, then re-run `owed_verification.py check --workdir "$PWD"` — the recorded verdict discharges the debt, and `clear --reason` is reserved for a round that could not run. Iterate `debts[]`, not `owed[]` / `dispatch_commands[verifier]`: those views are name-keyed and keep only the last row, so two commits owing one verifier would dispatch one audit against the wrong diff.
