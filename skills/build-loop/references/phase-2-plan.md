...
3g. **Risk-targeted acceptance plan**: For any non-trivial code change, load `references/targeted-acceptance.md`, write or reuse `.build-loop/runs/<run_id>/acceptance-lanes.json`, add the single machine-readable `acceptance_context` block to `.build-loop/runs/<run_id>/plan.md`, and run `scripts/acceptance_selector.py` with every expected changed file, criterion ID, named risk, and that run-local plan. Persist its compact output at `.build-loop/acceptance-selection.json` and copy the selected lane IDs plus their reasons into the plan. Missing manifest or context, substitute run paths, exit 2, context/criterion mismatch, or `verdict: incomplete` blocks Execute until the manifest covers the missing target or a declared full-suite fallback does. A full suite belongs in the plan only when the selector records `release_boundary`, `force_full`, `required_on:<boundary>`, or `fallback_for_uncovered:<targets>`; reassurance alone is not an acceptance reason.
...
- Acceptance lanes that do not cover every changed path, criterion, and named risk? Any full-suite command without a selector reason?
