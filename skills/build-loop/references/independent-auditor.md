...
**The cross-vendor round is a separate debt, not a stronger auditor.** When `scripts/review_trigger.py` sets `cross_vendor_required` on the run's profile, the run owes a review round from a different vendor in addition to this auditor. `scripts/owed_verification.py` arms that as `cross-vendor-audit` on the same run-record write that arms the auditor debt, and only a rendered verdict whose `judge_id` contains `cross-vendor` discharges it. A `yay` from this agent does NOT: on run `bl-20260912T180923Z-claude_code-selfmodrevert` the skipped cross-vendor round later returned 11 findings, 6 Critical, disjoint from this auditor's own 11. `scripts/run_close_lint.py` reports `review_owed` (exit 1) while either debt is outstanding, so a report cannot close over it.
