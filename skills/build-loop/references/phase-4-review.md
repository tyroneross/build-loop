...
1. **Dispatch `independent-auditor`** at `scope: "build"` against the full build diff (`<pre_build_sha>..HEAD`). Consolidated 2026-05-23 — single source of truth replacing both retired `commit-auditor` (chunk + build scope) and earlier retired `sonnet-critic`. The auditor has tools=[Read, Grep, Glob, Bash] (Bash for `git diff`), no Edit/Write.
   ...
   **The `cross-vendor-audit` debt arms itself.** Step 0's profile already computes `cross_vendor_required`; `scripts/owed_verification.py` now owes that round back instead of leaving it an advisory note. `enforce_for_run_record` arms `cross-vendor-audit` on the same run-record write that arms the auditor debt — from a recorded `review_trigger.cross_vendor_required: true`, or by re-deriving the profile from the record's `filesTouched`. Discharge requires a rendered verdict whose `judge_id` contains `cross-vendor` (canonical: `cross-vendor-audit`), stamped with the debt's `run_id` and pinned `diff_range`, naming a `vendor` outside the run's own host family, and carrying an `evidence` path that exists on disk — the vendor string alone is unauthenticated metadata the recorder asserts about itself. The same-vendor `independent-auditor` verdict deliberately does not discharge it. `check` re-reads the ledger and discharges the debt on that verdict; a rejected entry is reported with the field that disqualified it. `scripts/run_close_lint.py` returns `review_owed` (exit 1) while it is owed, so Report cannot close over it. When no peer host can run the round, `owed_verification.py clear --verifier cross-vendor-audit --run-id <run_id> --reason "<why>"` records the skip (audit log + `state.json.review_cleared_verifiers`, remembered per run_id) rather than hiding it — this is the honest form of the `cross_vendor: untested` note.

   **Parent-dispatch contract (machine-checkable).** A run or commit whose envelope carries `auditor_status: not-run:parent-must-dispatch` (or `cross-vendor-deferred`) — equivalently, one for which `owed_verification.py check` reports `status: incomplete` — is **NOT review-complete**. The dispatching parent — the top-level session that *does* have the Agent tool (the `/build-loop:run` skill body, or the human-driving session) — MUST, on receiving such an envelope: (1) `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/owed_verification.py check --workdir "$PWD" --json` to read `debts[]` — one row per (verifier, run_id) with its own diff_range and dispatch command; (2) for each owed verifier, dispatch it (`Agent(subagent_type="build-loop:independent-auditor")` on the run's diff range `<pre_build_sha>..HEAD`, plus `plan-critic` / `security-reviewer` when owed) and append its verdict to `.build-loop/judge-decisions.json`; (3) re-run `owed_verification.py check` — a verdict stamped with that debt's `run_id` and `diff_range` discharges it automatically, and `check --json` names the field that disqualified any entry it rejected; the last discharge removes the manifest and flips `review_incomplete` back to `false`. `clear` is the WAIVER, for a round that could not run: it requires `--reason`, refuses an unscoped clear when more than one run owes the verifier, and requires `--i-mean-every-run --reason` for an `--all` across several owners; (4) re-run `write_run_entry --scope build` so the review-completeness gate passes, and only then finalize Report. Two structural backstops now bind: the manifest (`owed_verification.py check` exit 1 on any owed verifier) AND the existing `write_run_entry --scope build` `review_completeness_error` → exit 3 on a `pass`+`scope=build`+files-touched run lacking a real auditor verdict. Neither can be satisfied by an honest nested orchestrator, which is what forces the parent to finish the audit instead of shipping un-audited code. Helper + tests: `scripts/owed_verification.py` / `scripts/test_owed_verification.py`.
...
**Silent-failure guard test — mutation-verify (any change touching a guard that fails without raising)**:

**Trigger** — a guard fails SILENTLY when a break yields a wrong-but-well-formed result instead of an error. Two shapes qualify, verified on identical terms: a guard that decides what a write is ALLOWED (a "never modify X" / "never delete Y" / "reject a path outside Z" boundary: an `isSidecarPath` write-fence, a path allowlist, a destructive-op gate), and a guard that decides what a read RETURNS (a filter, an exclusion predicate, a status or visibility check).

**Test shape** — the test MUST import and exercise the SHIPPED guard, never a copy of its logic pasted into the test. A test that asserts against its own duplicate passes even when the shipped guard is broken to `return true` — a test that cannot fail blocks nothing.

**Where to plant the mutation** — derive the target from what the guard's filter EXCLUDES, not from a convenient example. Mutation-verify it: flip the shipped guard to the wrong verdict, re-run only that test, confirm it now FAILS, restore. A dying mutant inside the guard's existing coverage proves the assertion is wired, not that the guard covers the defect.

**Observed:** an Obsidian plugin's sole never-modify-`writing/` guard had a test that re-implemented the guard, so it stayed 20/0 green when the real guard was mutated to always-allow; a store's `neighbors()` excluded rows whose OWN `verification_status == 'rejected'` (the rejection record, not the withdrawn claim), so a retracted assertion kept being served by 8 of 9 read paths behind a test asserting "no returned row has status 'rejected'" — trivially true whether or not retraction worked; separately, a guard checked `extraction_method='wikilink'` rows for fabricated confidence, a planted mutation on a wikilink row died and the guard was recorded verified, while all 3,897 real synthesized confidence values sat on `frontmatter` rows the guard could not see. A guard test that stays green against a broken guard, or that only ever mutates inside its own blind spot, is a fake test — treat it as a Validate failure (route to Iterate: import the real guard, retarget the mutation at the exclusion, then re-mutation-verify). **How** (one line): flip the guard's return in place → re-run its test → expect red → `git checkout -- <guard-file>`. Memory: `feedback_verify_test_validity_by_mutation`, `feedback_guard_test_written_against_implementation`, `mutation-test-must-target-the-guards-blind-spot`. No new script/agent — the rule + memory are the mechanism; a deterministic mutation-runner is a possible follow-up, not built here.
...
**Self-recursive builds:** when `selfRecursive.enabled == true` (the build is editing build-loop itself), Sub-step E also consumes `self_review.py`'s `self_simplification[]` findings as an additional hotspot source. Any simplification applied to build-loop's own code from this list MUST pass `python3 scripts/self_mod_verify.py --scope full --changed-files <the files you changed> --auto-revert --json` (`verdict: pass`) before it is committed. A `verdict: fail` auto-reverts the change; it does not route to Iterate — move it to `.build-loop/proposals/needs-human/` instead.
...
python3 scripts/report_lint.py       <draft.md> --json    # style: headline shape, validation line, jargon, contrastive pivot, percentage-denominator, length
```
...
- `summary.total > 0` → auto-revise the draft ONCE to clear the findings (translate jargon to plain language per the contract's blocklist, rewrite a missing headline as a one-sentence statement of what changed, add a validation line naming the exact command/method that verified the work, remove contrastive-pivot constructions, for a percentage-denominator finding verify which population the percentage came from and name it in the sentence — do NOT change the number itself unless the mismatch is real), then re-run the lint. If a second pass still has findings, emit the report with a `[warn] report-lint findings remain after one revise pass` line in `## Done` and continue. Never block on style.
