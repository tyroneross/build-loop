...
18. **Synthesis-density routing** (NEW 2026-05-07 — Phase 1 routing rule): if a plan file already exists for this build (re-evaluate at the end of Phase 2 if not), count its `synthesis_dimensions:` entries. **Reuse the existing parser** in `scripts/plan_verify.py` — call the `count_synthesis_dimensions()` helper (extracted from `rule_synthesis_dim_vague_value`'s block walker so the lint and density router share one parser). Do NOT write a second `synthesis_dimensions:` parser. **Threshold: count > 5 (6 or more entries) escalates the commit to `tier: thinking`** (single-context, Opus-class). Write the result to `state.json.synthesisDensity` as `{count, escalated, reason}`. The routing target is `tier: thinking`, **never a hardcoded model name** — the identifier resolves through the same tier abstraction used by the C5 halt-and-ask resolver (config override → orchestrator frontmatter → fail-loud). Effect on Phase 3 dispatch: when `escalated == true`, do NOT fan out to Sonnet implementers; execute the chunks inline at `tier: thinking`. The deterministic vague-value lint, the subjective synthesis-critic, and the C5 halt-and-ask backstop each grade one decision at a time; a 6+ dimension plan is synthesis-dense at the commit level, where fan-out loses cross-dimension coherence even when individual dimensions are well-specified. See `agents/build-orchestrator.md` Phase 1 §"Synthesis-density routing" for the orchestrator-side procedure.
