...
**System-access requests — ask once, check before retrying**: before any command or app launch could trigger a macOS Keychain, administrator, privacy, or sign-in dialog, consult `scripts/system_access_request.py --check-only` with a stable `--scope` naming the access need and a specific `--purpose` and `--requester`. The check reads the request ledger only. An existing pending, denied, cancelled, failed, or expired request is a hold: cite its request ID and existing note, continue independent work, and do not send another question or launch another requester. Changing a script, binary path, wording, agent, worktree, or run does not create a new access need. Read [the system-access protocol](references/system-access-once.md) before the first request or retry. This includes indirect `SecItemCopyMatching` calls inside Swift/Python helpers, app startup and signing tools; read-only does not mean prompt-free. Never probe real app Keychain items through ad hoc interpreters. Use injected stores or fixtures for automated checks. The ledger is enforced by the wrapper; commands outside it remain a protocol coverage gap, especially on Codex.
