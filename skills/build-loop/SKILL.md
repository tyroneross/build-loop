...
**Parallel fan-out**: After dequeue, partition entries by `files_touched` into independent groups (no overlapping files). Up to 4 `implementer` subagents run in parallel per pass via the bundled `agents/implementer.md` (Sonnet 4.6, scoped tools=[Read, Write, Edit, Bash, Glob, Grep]) — the hard cap is from `~/.claude/CLAUDE.md` §Sub-Agents. Sequential groups process after the parallel batch completes. Each implementer takes one queue entry as input and returns structured JSON (status + files_changed + verifications). Status routing: `fixed`→mark done; `partial`→re-pass; `scope_breach`→ask user; `deferred_architecture`→Review-F; `evidence_stale`→regenerate via `ux_triage.py --clear`. Results re-enter Sub-step B for re-validation. For Validate failures (no queue entry), construct an inline plan in the same shape and dispatch the implementer the same way — the agent doesn't care whether the plan came from a file or memory.
