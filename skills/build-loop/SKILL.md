...
18. **Synthesis-presence routing** (REVISED 2026-05-07 round-4 — Phase 1 default-tier flip): if a plan file already exists for this build (re-evaluate at the end of Phase 2 if not), count its `synthesis_dimensions:` entries. **Reuse the existing parser** in `scripts/plan_verify.py` — call the `count_synthesis_dimensions()` helper. Do NOT write a second parser. **Threshold (post-experiment flip): count >= 1 (any synthesis_dimensions entry) escalates the commit to `tier: thinking`** (single-context, Opus-class). Sonnet fan-out remains the default ONLY for plans with empty or absent `synthesis_dimensions` blocks — truly mechanical work (formatter runs, dep bumps, mechanical renames, doc-only edits). Write the result to `state.json.synthesisDensity` as `{count, escalated, reason}`. The routing target is `tier: thinking`, **never a hardcoded model name** — the identifier resolves through the same tier abstraction used by the C5 halt-and-ask resolver (config override → orchestrator frontmatter → fail-loud). Effect on Phase 3 dispatch: when `escalated == true`, do NOT fan out to Sonnet implementers; execute the chunks inline at `tier: thinking`. **Why the flip:** n=6 A/B experiment (2026-05-07, `~/dev/research/topics/synthesis-decision-delegation/experiment-2026-05-07/`) showed Sonnet implementers surface ~40% of architectural decisions Opus implementers surface even with full scaffolding (synthesis_dimensions, attestation envelopes, halt-and-ask). Defaulting to `tier: thinking` for any synthesis_dimensions trades ~28% token cost for ~60% better recall on synthesis-class decisions. C3/C4/C5 backstops remain in place as defense-in-depth but are insufficient alone — β didn't reliably recognize architectural decisions as novel-class, so halt-and-ask never fired on them. See `agents/build-orchestrator.md` Phase 1 §"Synthesis-presence routing" for orchestrator-side procedure.
