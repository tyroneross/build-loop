...
2. Report the CURRENTLY RECOMMENDED model vs what is AVAILABLE: run `python3 scripts/resolve_agent_model.py <agent> --json` (or `model_resolver.py --segment <s> --tier <t> --json`) and read back `model` + `resolution_path`. On Cursor, `model` is the taxonomy id and `dispatch_model` is the fused Task slug (`--plain` prints `dispatch_model`). `--host cursor` expands to every provider that host can dispatch; it is not provider `cursor`.
3. If the user names a model the index does not know, OFFER to classify it via the existing host-LLM flow: `python3 scripts/classify_model_tier.py lookup <id>` returns a WebSearch query + parse rubric; you run the search, decide segment + tier, then `record <id> --tier <tier> --segment <seg> --provider <vendor> [--provenance verified]`. No vendor API call — you (the host LLM) do the interpretation. Host Task slugs that fuse model + effort (`gpt-5.6-sol-medium`, `cursor-grok-4.6-high-fast`, `claude-opus-5-thinking-high`) inherit from the peeled family via `model_taxonomy.model_meta` — a new *variant* of a known id is not unknown. A genuinely new family still needs lookup. To ingest this session's picker list, write `.build-loop/host-models.json` as `{"slugs":[...]}` (or set `BUILD_LOOP_HOST_MODELS`) and run `python3 scripts/classify_model_tier.py detect --workdir "$PWD"`.
