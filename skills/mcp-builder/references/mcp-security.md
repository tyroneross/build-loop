...
> Source note: the control split and decision-table cells below are specified by the build-loop plan that introduced this file. Where a specific NSA-paper claim is cited, treat it as `[per plan spec]` unless independently verified against the paper.
...
| CVE-2025-49596 (RCE) | A documented remote-code-execution class in MCP server handling — the canonical "untrusted input reaches an executor" failure |
...
3. **Insecure-deserialization hygiene.** No `pickle`, no `eval`-backed parsers, no `Function(...)` over inbound data. Deserialize only with schema-bound, format-restricted parsers. (Directly addresses the CVE-2025-49596 class.)
