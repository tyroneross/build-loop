...
A skill that walks a 15-item completeness checklist before producing a build-loop-compatible plan markdown, then verifies the output with both a deterministic checker and an adversarial critic before returning.
...
**Example (rejected):** `placement: "follow existing layout"` — vague, will be lint-flagged.
