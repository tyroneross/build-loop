...
**Enforcement:** `scripts/worktree_isolation_lint.py` is the regression artifact. It scans `~/Library/LaunchAgents/*autonomy*`/`*poller*`/`*watcher*` jobs whose `WorkingDirectory` is a live checkout (exit 1 = BLOCKER) and asserts the in-repo wake path stays notify-only — flagging both a `git commit` and a commit-less file-editing call (write/stage/restore) without worktree provisioning (rules `wake-path-grew-a-commit` / `wake-path-grew-a-file-edit`). Run it in Phase 1 Assess on self-recursive build-loop runs and before installing any background writer. Co-located test: `scripts/test_worktree_isolation_lint.py`.
...
- `backlog/<id>.md` — active deferred work items being drained. Each uses `templates/backlog-item.md` frontmatter incl. mandatory `repo` + `branch` segmentation keys, plus `classify` (SAFE|RISKY|DECISION|PRODUCTION), `effort` (XS|S|M|L|XL), `status`. Drained by Phase 5 Iterate alongside `issues/` and `ux-queue/`. The **durable, slug-segmented** backlog of record lives at `build-loop-memory/projects/<slug>/backlog.md` (build-loop-ONLY for build-loop). **Segmentation is binding: never mix repos in one tracker** — a cross-repo item belongs in its own repo's scope. Cross-repo disposition is now MECHANICAL, not a per-repo tracker: file the item as a repo-tagged task on the shared **Operations Center queue** via `python3 scripts/file_to_operations_center.py --repo <repo> --title <one-line> --json` (the single shared board is the queue of record, so do NOT invent a build-loop-side cross-repo tracker). Only build-loop's OWN findings stay in these local `.build-loop/` trackers. (The freeform `OPEN-ITEMS.md` was retired 2026-05-31 for accreting cross-repo rows.) Full lane/segmentation contract: `skills/build-loop/references/memory.md` §"Artifact lanes & segmentation"; cross-repo routing policy: `references/keep-going-policy.md`.
