...
- Build orchestrator agent (Opus, Thinking tier — coordination) coordinates phase execution and spawns parallel subagents. Phase 2 plan synthesis runs at **Frontier (Fable)** *when the Advisor dispatch ladder is gated in* (`synthesisDensity > 5`, `riskSurfaceChange`, `stakes >= medium`, or `dispatch_tier: frontier`) — via the `advisor` agent (Rung 1), a peer host (Rung 2), or an already-Fable session (Rung 0); when no stakes trigger fires or no dispatch path is reachable, the orchestrator synthesizes the plan inline on its own model and labels it honestly (Rung 3, today's behavior). The verification surface (plan-critic, scope-auditor, independent-auditor, …) runs at Fable. See `skills/build-loop/references/advisor-dispatch-ladder.md`. Fable runs Phase 1 Assess + Phase 2 Plan synthesis is the *aspiration the ladder makes true on high-stakes plans* — not an unconditional guarantee.
...
|---|---|---|---|
| **Frontier** | Fable 5 | Planning synthesis (Phase 1 Assess + Phase 2 Plan) **via the Advisor dispatch ladder when stakes-gated** — `advisor` agent / peer host / already-Fable session; honestly-labeled inline-Opus fallback otherwise (`references/advisor-dispatch-ladder.md`) — AND verification verdicts: plan-critic, scope-auditor, independent-auditor (chunk + build scope), fix-critique, fact-checker, security-reviewer, overfitting-reviewer, promotion-reviewer | Clears Thinking-tier contract AND benchmarks above the prior-generation Thinking ceiling on at least one of SWE-bench Verified / ARC-AGI / GPQA Diamond |
...
- Every write-handoff brief MUST include all six MECE fields (owns / does-not-own / interface-contract / integration-checkpoint / allowed-tools / denied-tools); linted via `scripts/brief_mece_validator.py`.
