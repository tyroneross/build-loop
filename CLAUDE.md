...
- Build orchestrator agent (Opus, Thinking tier — coordination) coordinates phase execution and spawns parallel subagents; Fable (Frontier tier) runs Phase 1 Assess + Phase 2 Plan synthesis and the verification surface
...
|---|---|---|---|
| **Frontier** | Fable 5 | Planning synthesis (Phase 1 Assess + Phase 2 Plan) AND verification verdicts: plan-critic, scope-auditor, independent-auditor (chunk + build scope), fix-critique, fact-checker, security-reviewer, overfitting-reviewer, promotion-reviewer | Clears Thinking-tier contract AND benchmarks above the prior-generation Thinking ceiling on at least one of SWE-bench Verified / ARC-AGI / GPQA Diamond |
