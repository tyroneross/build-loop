...
**Namespace rewrite.** All `claude-code-debugger:*` qualified-skill references inside build-loop were rewritten to `build-loop:*`. The bridge skills (`debugger-bridge`, `logging-tracer-bridge`) kept their orchestration logic but were retitled as internal coordinators (they no longer indirect to an external plugin). **As of v0.7.0 (2026-05-02) the two bridges were dissolved**: when-to-fire policy moved into `agents/build-orchestrator.md`, the verdict / direct-apply gate moved into `skills/debugging-memory/SKILL.md`, the parallel-assess escalation lever moved into `skills/debug-loop/SKILL.md`, and the ephemeral-by-default + Mechanism A/B + placement rules moved into `skills/logging-tracer/SKILL.md`. The orchestrator now invokes those skills directly — no bridge layer.
