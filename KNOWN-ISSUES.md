...
## Dependency cooldown: per-PM native config keys + npm has no native exclude (REVISED 2026-09-12, v0.11.2)

The supply-chain dependency cooldown (`scripts/inject_dependency_cooldown.py`) writes a different native key per package manager (verified empirically on npm 11.14.1, 2026-05-16; source: mcollina gist + npm `config ls -l` + Socket.dev; pnpm floor corrected 2026-09-12 against pnpm's own release notes):
...
| **pnpm ≥ 10.16** | `minimumReleaseAge` | **MINUTES** (`10080`) | `pnpm-workspace.yaml` | `minimumReleaseAgeExclude:` (YAML list) — read by every pnpm ≥ 10.16, including 11+ |
| **pnpm 10.16–10.x only [UNVERIFIED]** | `minimum-release-age` | **MINUTES** | `.npmrc` | (exclude carried in workspace yaml) — the injector writes this key on every pnpm ≥ 10.16 for compatibility. **[UNVERIFIED, 2026-09-12]**: no source substantiates that pnpm 10.16–10.x reads this `.npmrc` key at all — confirmed by direct test that pnpm 11.10.0 does **not** read it (inert there), and pnpm/pnpm release notes pages 1-3 show the kebab name only in v12.0.0-rc.9, and only as the CLI override `--config.minimum-release-age`, never as a documented `.npmrc` key. Kept, not removed, because the 10.16–10.x range can't be tested here and removing it risks dropping a gate that may work on that line; `pnpm-workspace.yaml` `minimumReleaseAge` is the key **proven** to work (verified live via `pnpm config get`) |
| **pnpm < 10.16** | **NONE** — no native cooldown of any kind; falls back to the PreToolUse hook |
...
**The v0.11.1 bug (fixed in 0.11.2): the npm false-positive fix was never applied to pnpm.** `minimumReleaseAge`/`minimumReleaseAgeExclude` did not exist before pnpm 10.16.0 (Sep 2025) — pnpm 9.x and 10.0–10.15 have no cooldown support at all — but the injector's pnpm write path never checked the pnpm version, so it wrote the (inert) key unconditionally and returned `enforced:true` regardless. Because `allowlist_mechanism` was hardcoded to `"native"`, the PreToolUse hook stood down on a project with **no working gate**. Compounding this: a from-scratch write on a project with no pre-existing `pnpm-workspace.yaml` produced a file containing ONLY the cooldown keys, with no `packages` field. pnpm < 10.16 requires a non-empty `packages` field ([pnpm/pnpm#8968](https://github.com/pnpm/pnpm/issues/8968)) or every subsequent pnpm command fails with `ERR_PNPM_INVALID_WORKSPACE_CONFIGURATION` — reproduced in the field (ross-labs-astro, run-319525, pnpm 9.0.0): `pnpm install --offline --frozen-lockfile` failed outright after the injector ran. The injector now detects the pnpm version (mirroring the npm detector); pnpm < 10.16.0 writes nothing and falls back to the hook (`allowlist_mechanism:"hook"`, `enforced:false`), and it seeds `packages: ['.']` whenever `pnpm-workspace.yaml` has NO `packages` field at all — not only when the injector is creating the file from scratch, but also when an existing file (left by a prior buggy run, or by `pnpm approve-builds`, which writes only `onlyBuiltDependencies`) is missing one, so an already-broken repo self-heals on the next run. On a from-scratch write the seeded line is the first content in the file; on a self-heal of an existing file, `_merge_lines` appends it after the existing keys (still valid YAML — just a different line order than the from-scratch case). An existing `packages` field is never added to, removed, or rewritten.
...
**Follow-up (2026-07, pool-consolidation Inc 4):** `logging-tracer-bridge` was folded back into `skills/logging-tracer/SKILL.md` as an internal "Extended capability — Coding Debugger escalation" section (preflight + delegations + graceful-degradation preserved verbatim). `Skill("build-loop:logging-tracer-bridge")` no longer resolves. Remaining bridges: `prd-bridge`, `api-registry-bridge`, `defenseclaw-bridge`, `ibr-bridge`. The escalation preflight is now tested via `test_bridge_preflight.py::test_non_bridge_escalation_hops_have_preflight` (ADR-02).
...
**Related.** Two further items surfaced this run and are referenced rather than re-filed here: `BUIL-TOOLING-m2b7cts2d0gqn1d1j6q56` (self_mod_verify --auto-revert destroyed another session's uncommitted work — its destructive half is FIXED: `--auto-revert` now requires an explicit `--changed-files` list, backs every reverted file up as a ref-anchored git blob first, and honours a `snapshot --baseline` of pre-run dirt. The item's OTHER named defect — broad `--scope auto` test selection pulling a peer's dirty files into this run's scope, so a peer's work-in-progress can still fail an otherwise-green run — is NOT fixed and is tracked separately) and `BUIL-RUN-LEDGER-m2b4bcfb37r3r3vjf6d12` (stale owed-verification manifest).
