...
| **pnpm 10.16–10.x only** | `minimum-release-age` | **MINUTES** | `.npmrc` | (exclude carried in workspace yaml) — the injector writes this key on every pnpm ≥ 10.16 for compatibility, but only the 10.16–10.x line actually reads it; pnpm 11+ reads `pnpm-workspace.yaml` alone |
