...
3. **Major user-impacting decision** — a product/platform-direction choice the user would want to own (e.g. Android vs iOS, a user-facing product direction). The plan marks these `user_impact: major`; the orchestrator surfaces them. Implementation-tradeoff DECISIONs do NOT surface. **Autonomous long-running timeout:** a surfaced gate-#3 question auto-resolves to its `recommended_default` if unanswered within `autonomy.questionTimeoutMinutes` (default 10) — `scripts/question_timeout.py` returns `take_default`, the choice is logged to `autonomousDefaults[]` + captured as a DECISION, and listed in the readback. Gates #1–#2 (production push, irreversible delete) **never** time out — `question_timeout.py` holds them indefinitely (`production_hold: true`).
