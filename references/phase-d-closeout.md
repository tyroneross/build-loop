...
4. **Merge ready work, close owned worktrees, inventory the rest** (the pre-planned deletion point; it may slide if Iterate continues):
   - Every run that provisioned a worktree already recorded `state.execution.deletion_point` (`phase: D`, `may_change: true`). Phase D is that point. Stop is not.
   - From the integrating/primary worktree after positive owner release, run once: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/closeout_ready.py --workdir "$PWD" --run-id <exact-run-id> --owner-released --json`
   - `closeout_ready.py` inventories every linked worktree, fast-forwards or merge-risk-clears this run's branch into local `main` when ready, then delegates exact-branch closeout to `collapse_run.py`. Leftover already-merged `.build-loop/worktrees/run-*` trees close the same way. Unready, dirty, contested, unattributed, or foreign worktrees stay in `open_items` and `.build-loop/closeout/<run-id>-open-items.json`.
   ...
   - `collapse_run.py` remains the only destructive closer. `closeout_ready.py` is the merge + inventory front door that always runs. Direct `collapse_run.py --workdir "$PWD" --run-id <exact-run-id> --branch <exact-branch> --strict --merged-only --owner-released --release-source phase-d-integrator --json` is still valid for one already-merged branch.
...
When collapse reports `retained` or `surfaced_unmerged` entries, or `closeout_ready.py` reports `open_items`, surface them and require an explicit later disposition. The open-item inventory is `.build-loop/closeout/<run-id>-open-items.json`. When a run created zero refs (typical solo-on-main run), emit one line: `Branch hygiene: clean — no run-created branches/worktrees; on main.` Do not derive the run-wide `closeout_status` solely from branch cleanup; `runs[N].branch_closeout` and its receipt are the branch-hygiene projection.
