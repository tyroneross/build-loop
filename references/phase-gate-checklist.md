...
3. **Self-recursion check** (Priority — plugin-developer dogfooding signal): run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/detect_self_recursive.py --workdir "$PWD" --runtime-root "$CLAUDE_PLUGIN_ROOT" --json` and write the result to `.build-loop/state.json.selfRecursive`. The detector checks signals in precedence order: (a) the `--runtime-root` arg (set above to the currently-loaded plugin root) — `self_recursive = realpath(runtime_root) == realpath(workdir)`; (b) `CLAUDE_PLUGIN_ROOT` from env, same check, when the arg is absent; (c) legacy fallback — walk `~/.claude/plugins/` for a symlink resolving to the workdir (covers ad-hoc dev symlinks under the legacy direct OR per-version cache layout). Regardless of method, `<workdir>/.claude-plugin/plugin.json` (with a `name`) and `<workdir>/.git/` must both be present. The result includes a `detection_method` field: `runtime_root_arg | plugin_root_env | cache_symlink | none`. When `self_recursive: true`, set `state.json.selfRecursive.enabled: true` and surface to the user in the Phase 1 Assess brief: "🔁 Self-recursive build detected — working copy is the runtime. Per-commit mode available via `/build-loop:run --per-commit`." When false, the `reason_if_false` field (one of `not_a_plugin | no_runtime_link | not_a_git_repo | symlink_check_failed`) is informational only — do not block. Per-commit dispatch itself is implemented in a downstream commit; this step only writes the detection result and surfaces the note. Background on the load mechanism (`claude --plugin-dir <path>` vs cache copy) and why symlinks under `~/.claude/plugins/` are fragile: `skills/build-loop/references/self-recursive-dev.md`.
