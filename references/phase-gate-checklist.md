...
- `## PARENT MUST DISPATCH` — emitted ONLY when `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/owed_verification.py check --workdir "$PWD" --json` reports `status: incomplete` (exit 1), i.e. a nested orchestrator (no Agent tool) wrote `.build-loop/owed-verification.json` at Sub-step A because it could not dispatch `independent-auditor` / `plan-critic` / `security-reviewer` itself. One row per owed verifier with the manifest's `dispatch_commands[verifier]` verbatim, plus "run is `review_incomplete: true` until owed verifiers return and are cleared." Non-optional top-level header — never fold into `## Held`. Run `outcome` stays `partial` (never `pass`) while owed. The dispatching parent dispatches each verifier, appends the verdict to `.build-loop/judge-decisions.json`, then `owed_verification.py clear --verifier <name>`; the last clear removes the manifest and flips `review_incomplete` false. Omit the section on `status: complete`/`absent`. This makes the GAP-1 parent-dispatch contract a gate rather than a memory.
