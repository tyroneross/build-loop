...
**Selection policy (Hybrid):** per `(segment, tier)` there is an ORDERED preferred-model list (order = capability rank, honoring Accuracy>Speed>Cost). The resolver (`scripts/model_resolver.py resolve_role`) picks the highest-ranked AVAILABLE + host-reachable id. It returns the selected `model`, host-compatible `preferred_models[]`, advisory `preferred_effort`, `resolved`, `source`, and the full `resolution_path`. Project and per-dispatch overrides remain preferences: a valid override wins, while an unavailable, host-unreachable, or known-below-floor choice falls through safely. Reorder durable preferences in `references/model-taxonomy.json`; use `.build-loop/config.json.modelOverrides` for a project choice. Host filtering prevents a Claude dispatch from receiving an OpenAI id and prevents a Codex dispatch from receiving an Anthropic id. Cursor is the exception: it is a host family, not a provider, and `--host cursor` expands to `{anthropic, openai, xai, cursor, google}` via `references/model-taxonomy.json` `hosts`. Treating Cursor as provider `cursor` made every Anthropic/OpenAI candidate `host_reachable: false`.
...
- **Effort default:** `high` for `agentic_execution/T3` on both hosts. A caller may select another supported effort for a bounded task; the resolver reports this as a preference rather than a lock.
...
| xAI | n/a — `grok-4.6` is a T3 `candidate-unverified` seed, not on a preferred list | n/a | `grok-4.6` (Cursor Task slug `cursor-grok-4.6-high-fast`; not preferred — recency would steal T3) | n/a |
| Cursor-native | n/a | n/a | Composer 2.5 (`composer-2.5`, Task slug `composer-2.5-fast`); not on a preferred list — Sonnet/Terra win when those providers are in the host set | n/a |
| Local (Ollama / MLX) | n/a — none meets contract yet | n/a — none meets contract yet | `qwen2.5-coder-32b` | `llama3.2-3b` |

### Cursor is a host, not a provider

Cursor dispatches Anthropic, OpenAI, xAI, Google, and Composer ids in one session. Resolve with `--host cursor` (or run inside Cursor so `detect_host` returns `cursor`); `scripts/host_model_map.py` expands that family to the provider set in `references/model-taxonomy.json` `hosts.cursor`. `resolve_agent_model.py` keeps the taxonomy id on `model` / `canonical_model` and puts the fused Task slug on `dispatch_model` (`opus` → `claude-opus-5-thinking-high`, `gpt-5.6-sol` + high → `gpt-5.6-sol-high`; `--plain` prints `dispatch_model`). Optional session allowlist: `BUILD_LOOP_HOST_MODELS` or `.build-loop/host-models.json` `{"slugs":[...]}` so dispatch picks a slug the current picker can actually send. New fused variants of a known family inherit; a new family still goes through `classify_model_tier.py lookup` / `detect`.
...
| **Terra** | `build-orchestrator`, `assessment-orchestrator`, `implementer`, domain assessors, `ui-validator`, `alignment-checker`, `synthesis-critic`, `retrospective-synthesizer`, `self-improvement-architect`, `leak-scanner` | `medium`; `high` for a complex bounded chunk or first retry | Move to Sol when the specification becomes ambiguous, the work becomes high-consequence, or repeated verified attempts fail. |
