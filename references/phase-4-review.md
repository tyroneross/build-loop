...
This writes `.build-loop/owed-verification.json` — the owed verifier list plus the exact `Agent(...)` dispatch command the parent must issue per verifier — and flips `state.json.review_incomplete = true`. The manifest is the machine-checkable substitute for the parent remembering. The dispatching parent (which HAS the Agent tool) reads the manifest's `debts[]`, dispatches each row against ITS OWN `diff_range`, appends the verdict to `.build-loop/judge-decisions.json`, then `owed_verification.py clear --verifier <name> --run-id <run_id>` (or `--all`, which clears every debt deliberately and needs no run scope); clearing the last debt removes the manifest and flips `review_incomplete` back to `false`. An unscoped `clear --verifier <name>` is REFUSED when more than one run owes that verifier. Colocated helper + tests: `scripts/owed_verification.py` / `scripts/test_owed_verification.py`.

**`cross-vendor-audit` — the second debt, armed automatically.** `scripts/review_trigger.py` computes `cross_vendor_required` on every Review-A profile, and a run whose diff sets it true owes a review round from a DIFFERENT vendor. `enforce_for_run_record` arms that debt at the same point it arms the auditor debt, so the orchestrator does not write it by hand: it fires when the run record carries `review_trigger.cross_vendor_required: true`, or when re-deriving the profile from the record's own `filesTouched` returns true. Only a rendered second-vendor verdict discharges it: `judge_id` must contain `cross-vendor` (canonical id `cross-vendor-audit`) AND the entry must carry a `vendor` field naming a RECOGNISED provider outside the run's own `host` family (an allowlist: `unknown` is not evidence, and an unmappable host cannot answer "different vendor" at all, so the debt stays armed). The verdict itself must be one of the valid judge verdicts and its `status` must not be a failure status — `verdict: "not-run"`, or `verdict: "yay"` riding `status: "failed"`, is a record of the round NOT happening. The emitted dispatch command spells the exact entry that discharges it (`judge_id` / `verdict` / `vendor` / `run_id` / `diff_range`), because following a command that produces an undischargeable entry left the waiver as the only visible exit from a round that had actually run — an id alone is a label an agent types, and the debt exists to force a different vendor. The verdict is read from the run record and from `.build-loop/judge-decisions.json`, because the round runs after the record is written; an entry there must name this `run_id` AND the `diff_range` it reviewed, since the file is append-only and an unstamped or obsolete verdict would otherwise discharge a debt armed by later commits. Ranges are compared after resolving both sides' endpoints, so the run's `<sha>..HEAD` and a reviewer's two concrete shas match — **a same-vendor `independent-auditor` verdict does not**, because the value of the round is precisely the findings the same-vendor auditor does not reach. Measured on run `bl-20260912T180923Z-claude_code-selfmodrevert`: the trigger returned true for a 1282-line diff, the round was skipped, the run reported clean, and running the round afterwards returned 11 findings (6 Critical) disjoint from the same-vendor auditor's own 11 — one of them a live path that globbed a caller pathspec and destroyed two files' uncommitted edits. When no peer host can execute the round, `owed_verification.py clear --verifier cross-vendor-audit --run-id <run_id> --reason "<why>"` records the skip in `.build-loop/audit-log.md` and in `state.json.review_cleared_verifiers` rather than hiding it; that manual waiver is remembered per run_id so a later write cannot silently re-arm it, and the clear result reports `tombstone_persisted: false` when the waiver could not be stored. A verdict-based discharge deliberately leaves NO waiver, so a later correction that widens the run's file set (which drops the now-stale verdict) re-arms the debt. `run_close_lint.py` blocks only the run that OWNS the debt (`review_owed`, exit 1); another run's outstanding debt is named at `review_owed_other_run`, exit 0, because refusing an unrelated close teaches the operator to waive. The manifest records one row per **(verifier, run_id)** in `debts[]`, because a debt is per run and a single slot per verifier name could not represent two runs owing the same round — the second write dropped the first run's obligation and that diff shipped unreviewed. Each debt carries its OWN `diff_range`, so a later run's write cannot re-point an earlier run's dispatch command at the wrong diff, and a waiver granted to one run never suppresses another run's obligation. `owed`, `cleared`, `dispatch_commands` and `owed_runs` are derived views kept for existing readers, and they are **lossy when two runs owe one verifier** — the name-keyed maps keep only the last row. Read `debts[]` for anything that decides a lifecycle: the discharge, the close gate's ownership check, and the `## PARENT MUST DISPATCH` rows all iterate the debt rows, so each outstanding debt prints its own run_id, diff_range and dispatch command. A manual waiver needs `--run-id` whenever more than one run owes the verifier; the CLI refuses an ambiguous clear rather than discharging a peer run's round on this run's excuse. A verdict read from `.build-loop/judge-decisions.json` must name this `run_id`: the file is append-only and outlives every run, so an unscoped read let an older verdict discharge a new debt. `owed_runs` — which run owes which verifier — because a manifest accumulates debts across runs and its `run_id` is only the last writer's; without ownership a low-risk run's own verdict discharged an earlier run's outstanding round.
