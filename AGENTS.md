...
| `LSP` (find callers of a symbol) | `mcp__code_intel__refs`, else grep | Available when the external `code-intel` MCP server is configured (`codex mcp list` to check). It needs tool approval, so under `--ask-for-approval never` the call is DENIED and you must fall back to grep — record which one you used, per `agents/scope-auditor.md`. |
